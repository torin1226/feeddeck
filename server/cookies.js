...
try {
  mkdirSync(COOKIES_TMP, { recursive: true })
} catch (err) {
  if (err.code !== 'EEXIST') logger.warn(`Failed to create cookie temp dir: ${err.message}`)
}
...
try {
  ...
  logger.warn(`Failed to clean stale cookie temps: ${err.message}`)
}
