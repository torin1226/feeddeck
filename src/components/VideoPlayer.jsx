...
export default function VideoPlayer({ video, onClose, onPlayVideo }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/stream-formats?url=${encodeURIComponent(video.url)}`)
      ...
      .catch(() => {})
    // eslint-disable-next-line react-hooks/exhaustive-deps -- selectedQuality changes trigger handleQualityChange instead
  }, [video.url, isSFW])

  // Mark video as watched on mount
  useEffect(() => {
    markWatched(video.id)
    // eslint-disable-next-line react-hooks/exhaustive-deps -- markWatched is a stable store action
  }, [video.id])
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- handleNext uses advance/onPlayVideo which are stable; including it would need useCallback
  }, [video])
