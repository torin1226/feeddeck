...
export default function NextUpDialog({ videoRef, nextVideo, onAdvance }) {
  ...
  if (visible && !nextVideo) {
    return (
      <div
        className="absolute bottom-20 right-4 z-toast w-[min(280px,calc(100vw-2rem))] rounded-xl bg-black/60 backdrop-blur-xl border border-white/10 shadow-modal p-4 text-center"
  ...
  return (
    <div
      className="absolute bottom-20 right-4 z-toast w-[min(280px,calc(100vw-2rem))] rounded-xl bg-black/60 backdrop-blur-xl border border-white/10 shadow-modal overflow-hidden cursor-pointer hover:border-white/20 transition-colors"
      style={{ animation: 'foryou-slideInRight 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' }}
      onClick={onAdvance}
    >
      {/* Thumbnail */}
