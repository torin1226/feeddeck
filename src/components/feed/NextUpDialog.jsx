...
export default function NextUpDialog({ videoRef, nextVideo, onAdvance }) {
  ...
  if (visible && !nextVideo) {
    return (
      <div
        className="absolute bottom-20 right-4 z-50 w-[280px] rounded-xl glass border border-highlight shadow-2xl p-4 text-center border-t-highlight-medium"
  ...
  return (
    <div
      className="absolute bottom-20 right-4 z-50 w-[280px] rounded-xl glass border border-highlight shadow-2xl overflow-hidden cursor-pointer hover:border-highlight-strong transition-colors border-t-highlight-medium"
      style={{ animation: 'foryou-slideInRight 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' }}
      onClick={onAdvance}
    >
      {/* Thumbnail */}
