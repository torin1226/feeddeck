...
- [x] (2026-03-28) Hover preview video element cleanup — root cause: bare `useHomeStore()` in CategoryRow subscribed to entire store, causing React re-renders that reset video `style.opacity` to 0 mid-playback. Fixed: all 35 bare store subscriptions converted to field-level selectors across 24 files. Also added useEffect cleanup for singleton DOM reference in `useHoverPreview.js`
