...
| Discovered Tasks | Mixed | 44/50 (88%) | Editorial design polish, deferred items |
...
- [ ] (2026-04-30, P3) **Tighten the 10-min readiness validity window if false-positives observed** — `data/.warm-complete` mtime gates the readiness probe. 10 min is a guess. If users observe "ready" while cache is actually empty post-long-downtime, tighten to 5 min. If the probe still flaps in dev despite persistence, widen to 30 min. Currently no signal either way; revisit after a week of real use.
...
- [x] (2026-05-01) **Discovered: Orphan NSFW category keys cleanup** — Replaced the obsolete "Diversifying NSFW categories" Phase 1.4 seed block in `server/database.js` (was line 510-526) with an idempotent cleanup migration. Root cause: the seed used `INSERT OR IGNORE` to add `nsfw_redgifs_pov`, `nsfw_redgifs_solo`, `nsfw_xvideos_new`, `nsfw_xvideos_hits`, `nsfw_spankbang_new`, `nsfw_fikfap_new` on every boot; the 2026-04 NSFW row layout migration drops these (line 964-968) but only runs once (gated on `nsfw_for_you` marker), so on every subsequent boot the seed re-inserted them as inert orphans (no `topic_sources`, no inserter). New block runs `DELETE FROM homepage_cache WHERE category_key IN (...)` + `DELETE FROM categories WHERE key IN (...)` for the 6 keys — idempotent (a no-op on clean DBs), tiny (2 prepared statements per boot), and removes the seed source so they can't reappear. Build clean (`built in 1.90s`); 0 new ESLint warnings (3 pre-existing in unrelated files). User-invisible bug — `/api/homepage` already filtered them out — but tidies the categories table and quiets the warm-cache "0 new videos" log noise on those keys. Verified no other callers in `src/` or `server/`.

- [x] (2026-04-30) **Homepage: kill placeholder-dogs bug — five-layer fix** — Commit `58bbe6c`. (1) Deleted obsolete `nsfw_trending` migration block in `database.js` that fired a wipe loop on every boot (the new `nsfw_for_you` layout migration drops the marker key, so the guard kept firing → `DELETE FROM homepage_cache` + reseed → drop again). This was the actual root cause of why the cache was empty whenever users opened `/home`. (2) Wrapped `social_news` + `nsfw_for_you` layout migrations in `BEGIN`/`COMMIT`/`ROLLBACK` so an interrupted run can't half-migrate. (3) `/api/homepage` now returns `state: 'warming'|'ready'` so the client doesn't infer warming-state from `categories.length`. (4) `homeStore.fetchHomepage` no longer calls `generateData()` on empty/error — instead sets `homepageState='warming'` and schedules an exponential-backoff retry (1s→2s→4s→8s, cap 15s, max 12 attempts). HomePage already shows skeletons off `!heroItem`. `resetHome` cancels pending retries. `generateData()` preserved for design preview only. (5) New `/api/health/ready` endpoint returns 503 until first warm-cache completes AND `homepage_cache` has fresh-unviewed rows, persisted to `data/.warm-complete` (file mtime, 10-min validity) so `node --watch` (dev) and fast restarts (prod incidents) don't flap the readiness probe. Plus `src/__tests__/homeStore-empty-response.test.js` (7 tests, greps for breed names — future regressions fail loudly). Tests 159 → 166. Lint clean. Build 2.90s. Verified live: real YouTube content renders, no dog placeholders. Architectural rationale + how-to-apply notes in `_memory/decisions/2026-04-30-homepage-readiness-and-self-healing.md`.
