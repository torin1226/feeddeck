...
- [ ] (2026-04-30, P3) **Orphan NSFW category keys in `categories` table** — `nsfw_redgifs_pov`, `nsfw_redgifs_solo`, `nsfw_xvideos_new`, `nsfw_xvideos_hits`, `nsfw_spankbang_new`, `nsfw_fikfap_new` are leftovers from earlier wipe-loop cycles (each restart pre-fix re-seeded defaults from `_seedCategories`, then the NSFW layout migration dropped the new layout but left these orphans). They have no `topic_sources`, no inserter targets them, and `/api/homepage` filters out empty categories — so they're inert. Cleanup: add an idempotent one-shot `DELETE FROM categories WHERE key IN (...)` for these specific keys, run once on next boot, gone. Low priority because user-invisible.
