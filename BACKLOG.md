# FeedDeck Backlog
<!-- COWORK TEST: pineapple-rocket-42 -->

**Repo:** https://github.com/torin1226/feeddeck

This is the single source of truth for all project tasks. Claude Code and Cowork read from and update this file directly.

For backlog management protocol, see `BACKLOG_SKILL/SKILL.md`.

---

## Status Key

- `[ ]` — Not started
- `[~]` — In progress
- `[x]` — Complete
- `[!]` — Blocked (see notes)
- `[?]` — Needs decision from user

---

## Milestone 1: Desktop MLP (Minimum Lovable Product)

> Goal: Open the site on desktop, browse real content, watch videos, queue stuff, and it feels polished. Everything below this line ships before moving to mobile.

### 1.1 Finish Homepage Animation

- [x] Create `useFeaturedScroll.js` hook with state machine (IDLE, SCROLLING, SNAPPING, REVEALED)
- [x] Implement 250vh scroll zone wrapper with sticky inner container (100vh)
- [x] 3-phase animation: Phase A (enter small→fullscreen), Phase B (hold fullscreen), Phase C (fullscreen→carousel)
- [x] Active card uses `transform: scale(X)` — never set width/height during scroll
- [x] Side cards emerge via opacity and translateX in Phase C only
- [x] Border-radius transitions, tipping point snap, REVEALED↔SCROLLING transitions
- [x] Fix stale closure bugs (refs-only approach, no useCallback)
- [x] Fix late-mount initialization (totalCards watcher for async data)
- [x] Test in real browser and fine-tune phase boundaries/timing
- [x] Add `will-change: transform` on cards for GPU compositing hints

### 1.2 Wire Homepage to Real Data

- [x] Create `playerStore.js` for shared activeVideo state across hero, carousel, queue
- [x] Theatre mode end-to-end: hero video plays, TheatreControls work, CategoryRows hide, exit restores
- [x] Hero ↔ Carousel interaction: clicking carousel card updates hero background/title/metadata
- [x] Homepage → real data bridge: replace picsum.photos placeholders with data from library/backend
- [x] Verify all keyboard shortcuts work on HomePage (not just LibraryPage)

### 1.3 Backend Homepage Endpoints

- [x] Add `homepage_cache` table (id, category, mode, url, title, thumbnail, duration, source, uploader, fetched_at, expires_at, viewed)
- [x] Add `categories` table (key, label, query, mode where mode = 'social' or 'nsfw')
- [x] `GET /api/homepage?mode=social|nsfw` returns cached videos grouped by category
- [x] Async yt-dlp refill when any category drops below 8 valid videos
- [x] `POST /api/homepage/viewed?id=` marks video viewed, triggers refill if below threshold

### 1.4 Hover Preview

- [x] Stream real video (muted, low-res) on thumbnail hover
- [x] 300ms debounce before starting stream
- [x] Abort stream on mouseout
- [x] Max 1 preview streaming at a time (new hover cancels previous)
- [x] No duration cap — plays as long as user hovers

### 1.5 Error Handling & Resilience

- [x] Stale streaming URL detection and refresh (yt-dlp URLs expire)
- [x] yt-dlp failure handling: video taken down, geo-blocked, rate limited — surface clear error to user
- [x] Loading skeletons for grid, hero, carousel, featured section (not spinners)
- [x] Empty states: no videos in library, empty queue, empty search results, no categories loaded

### 1.6 Rebrand Cleanup

> Internal plumbing still uses old "Puppy Viewer" names. Fix before shipping. 20 files affected, ~70 references total.

**localStorage & Zustand stores:**
- [x] Rename localStorage keys: `pv-mode` → `fd-mode`, `pv-lib` → `fd-lib`, `pv-queue` → `fd-queue`, `pv-app` → `fd-app`, `pv-content` → `fd-content`
- [x] Add migration: on load, if old `pv-*` keys exist, copy to `fd-*` keys and delete old ones
- [x] Update Zustand store `name` fields in: `modeStore.js`, `libraryStore.js`, `queueStore.js`, `useAppStore.js`, `useContentStore.js`
- [x] Update `DebugPanel.jsx` localStorage key references (`pv-mode`, `pv-lib`, `pv-queue`)

**Puppy data removal (entire SFW→Social rewrite):**
- [x] Delete `src/data/puppyData.js` entirely (35 puppy references)
- [x] Remove all `import { getSFWData } from '../data/puppyData'` in: `VideoPlayer.jsx`, `FloatingQueue.jsx`, `QueueSidebar.jsx`, `VideoCard.jsx`, `Queue.jsx`
- [x] Remove `import { getSFWData, generateDemoPuppies } from '../data/puppyData'` in `MobileSwipeView.jsx`
- [x] Replace puppy-specific SFW rendering logic with dual-mode Social data in all components above

**UI branding:**
- [x] `Header.jsx`: "Puppy Gallery" → "FeedDeck", remove puppy branding
- [x] `ModeToggle.jsx`: "Switch to puppy mode" → appropriate Social mode label
- [x] `useAppStore.js`: `document.title = 'Puppy Gallery 🐕'` → `'FeedDeck'`
- [x] `modeStore.js`: Remove `PUPPY_FAVICON`, update title logic to always show "FeedDeck" / 📡
- [x] `VideoPlayer.jsx`: Remove hardcoded puppy pexels video URL, replace SFW logic
- [x] `VideoGrid.jsx`: "Check back later for more puppy videos!" → appropriate empty state
- [x] `QueueSidebar.jsx`: "Queue up some puppy videos!" → appropriate empty state
- [x] `dist/index.html`: "Puppy Gallery 🐕" → "FeedDeck"

**Project-level:**
- [x] `package.json`: `"name": "puppy-viewer"` → `"feeddeck"`
- [x] `.claude/launch.json`: rename both config entries from "puppy-viewer" to "feeddeck"
- [x] Rename project folder from `puppy-viewer/` to `feeddeck/` (coordinate with user)
- [x] Update all import paths after folder rename

### 1.7 Desktop MLP Polish

- [x] Responsive breakpoint audit: verify homepage at all Tailwind breakpoints (sm/md/lg/xl)
- [x] Next-up preview overlay: show upcoming video in corner before queue auto-advances
- [x] Playback speed selector (0.5x to 2x)
- [x] Quality selector — deferred to Milestone 3 (requires `yt-dlp -F` endpoint)
- [x] Hero search bar: make it narrower, move helper text from beside it to below it

---

## Milestone 2: Swipe Feed

> Goal: A buttery-smooth infinite vertical video feed that aggregates short-form content from multiple sources. Available on mobile AND desktop at `/feed`. Think TikTok but pulling from everywhere, with the mode (Social vs NSFW) determining which sources are mixed in.

### Design Spec

#### Gesture Map (touch + trackpad/scroll on desktop)

| Gesture | Action |
|---------|--------|
| Swipe up | Next video |
| Swipe down | Previous video |
| Swipe right | Open source URL in new browser tab |
| Swipe left | Add to queue (animation + toast confirmation) |
| Double-tap | Like/heart (burst animation) |
| Tap | Play/pause toggle |
| Long-press | Source controls: "More from this source" / "Hide this source" |

#### Feed UI

- **Overlay (subtle, always visible):** Video title, creator name, source icon (small, bottom-left)
- **Timeline bar:** Thin scrubbable progress bar at very bottom, TikTok-style. Shows position in current video.
- **Bottom nav bar:** 4 tabs — Feed, Search, Queue, Profile/Settings
- **No sidebar icons.** All actions are gesture-driven. Clean, immersive.

#### Video Display

- **Vertical video:** Full-bleed, fills viewport
- **Horizontal video (default):** Cropped/fit to fill — no black bars, center-crop to fill vertical frame
- **Horizontal video (user setting):** Letterbox mode — black bars top/bottom, video at native aspect ratio
- **Setting location:** Profile/Settings tab, "Video Display" section

#### Feed Algorithm

- **Sources per mode:**
  - Social: YouTube Shorts, Instagram Reels, TikTok
  - NSFW: fikfap, pornhubshorties, and similar aggregators (user can add more)
- **Mix:** Library videos (previously saved) shuffled in with discovery (new from sources)
- **Max length:** User-configurable in settings (default: no limit)
- **Source weights:** Algorithm-driven, influenced by "more from this source" / "hide this source" feedback
- **No repeat:** Don't show the same video twice in a session unless user scrolls back

#### Source Management

- **In-feed:** Long-press any video → "More from this source" (boost weight) or "Hide this source" (suppress)
- **Backend:** `sources` table tracks domains, mode, weight, active/hidden status
- **Adding sources:** Future — in-app setting to add new domains. For MLP, hardcode the initial source list.

#### Content Pipeline

- **Scheduled background fetch:** Cron-style job fetches new videos from each active source on an interval (e.g., every 30 min). Stores metadata + streaming URL in `feed_cache` table.
- **On-demand top-up:** When the client's buffer drops below 5 unwatched videos, hit `GET /api/feed/next?mode=X&count=10` to fetch more.
- **Expiry:** Streaming URLs expire. Backend tracks `expires_at` and re-fetches via yt-dlp when needed.
- **Dedup:** Same video URL from different aggregators = one entry.

### 2.1 Feed Core

- [x] Create `/feed` route, accessible from both mobile and desktop
- [x] Full-viewport vertical scroll container with CSS scroll-snap (`snap-y snap-mandatory`)
- [x] Each video fills 100vh (or 100dvh for mobile browser chrome)
- [x] Infinite scroll: fetch next batch when 5 videos from end of buffer
- [x] Video autoplay on snap (muted initially, unmute on first tap)
- [x] Preload next 2 videos, cache previous 2, unload beyond that
- [x] Vertical video: full-bleed, no transforms needed
- [x] Horizontal video: default center-crop to fill frame (`object-fit: cover`)
- [x] Horizontal video letterbox setting: `object-fit: contain` with black background
- [x] Store letterbox preference in user settings (localStorage for now, server later)

### 2.2 Gesture System

- [x] Swipe up/down: snap to next/previous video (scroll-snap handles this natively)
- [x] Swipe right: detect horizontal swipe threshold (>50px, <30deg angle), open `video.sourceUrl` in new tab
- [x] Swipe left: detect horizontal swipe, trigger add-to-queue
- [x] Swipe-left feedback: card slides left with queue icon animation + toast "Added to queue"
- [x] Double-tap: like/heart with burst animation overlay (heart particles, TikTok-style)
- [x] Tap: play/pause toggle
- [x] Long-press (>500ms): open source control sheet ("More from this source" / "Hide this source")
- [x] Desktop: map scroll wheel to swipe up/down, keyboard arrows too
- [x] Desktop: map left/right arrow keys to swipe-left (queue) and swipe-right (source)
- [x] Prevent gesture conflicts: horizontal swipes only register if clearly horizontal (angle check)

### 2.3 Feed UI Components

- [x] `FeedPage.jsx`: route component, manages feed state and infinite scroll
- [x] `FeedVideo.jsx`: single video card (100vh), handles video element + overlay
- [x] `FeedOverlay.jsx`: subtle bottom-left overlay — title, creator, source icon (built into FeedVideo)
- [x] `FeedTimeline.jsx`: thin scrubbable progress bar at bottom edge (built into FeedVideo)
- [x] `FeedBottomNav.jsx`: 4-tab navigation (Feed, Search, Queue, Profile)
- [x] `FeedToast.jsx`: transient notification for queue add, like confirmation
- [x] `HeartBurst.jsx`: double-tap heart animation (CSS particles or canvas)
- [x] `SourceControlSheet.jsx`: long-press bottom sheet with source actions
- [x] `QueueSwipeAnimation.jsx`: card-slides-left animation component

### 2.4 Feed Store & State

- [x] Create `feedStore.js` (Zustand): current feed buffer, current index, loading state
- [x] Feed buffer: array of video objects with `id`, `url`, `streamUrl`, `title`, `creator`, `source`, `sourceUrl`, `duration`, `orientation`, `thumbnail`
- [x] Track watched IDs in session to prevent repeats
- [x] Source weight map: `{ domain: weight }` persisted in localStorage/server (server-side via source-feedback endpoint)
- [x] User settings: max video length, letterbox preference

### 2.5 Feed Backend

- [x] `feed_cache` table: id, source_domain, mode, url, stream_url, title, creator, thumbnail, duration, orientation (vertical/horizontal), fetched_at, expires_at, watched
- [x] `sources` table: domain, mode (social/nsfw), label, weight, active (boolean), added_at
- [x] Seed initial sources: YouTube Shorts, TikTok (social); PornHub (nsfw)
- [x] `GET /api/feed/next?mode=X&count=10` — return next unwatched videos, weighted by source preferences
- [x] `POST /api/feed/watched?id=X` — mark video watched
- [x] `POST /api/feed/source-feedback` — body: `{ domain, action: 'boost' | 'hide' }` — adjust source weight
- [x] Background fetch job: iterate active sources, yt-dlp extract new videos, insert into feed_cache
- [x] Configurable fetch interval per source (default 30 min)
- [x] Stream URL refresh: if `expires_at` passed, re-fetch via yt-dlp before serving
- [x] Dedup: unique constraint on video URL, skip duplicates across sources

### 2.6 Mobile Testing via Local WiFi

> Test swipe feed and mobile UI on a real phone before any Pi/Tailscale setup. All M1-M3 work targets the local Windows laptop.

- [x] Update `vite.config.js` to bind dev server to `0.0.0.0` (not just localhost)
- [x] Verify Vite proxy still routes `/api` to Express for mobile clients
- [x] Update Express to also listen on `0.0.0.0`
- [x] Add dev script or console log that prints local network URL (e.g. `http://192.168.x.x:3000`) on startup
- [x] Document setup in a short note: connect to same WiFi, open URL on phone (Vite prints network URL on startup)

### 2.7 Feed Polish

- [x] Smooth snap animation (not jarring — ease-out, ~300ms) (CSS scroll-snap + smooth behavior)
- [x] Loading skeleton while next batch fetches
- [x] "You're all caught up" state when feed exhausted (rare but handle it)
- [x] Pull-to-refresh at top of feed (mobile)
- [x] Orientation transition: smooth crossfade when switching between vertical and horizontal videos
- [x] Respect system reduced-motion preference (disable burst animations, simplify transitions)
- [?] Test on real mobile devices: iOS Safari, Android Chrome (viewport, scroll-snap, gesture conflicts)
  > QUESTION: Ready for manual mobile testing. Start `npm run dev` and open the network URL on your phone (same WiFi). Test swipe up/down, left/right gestures, double-tap hearts, and long-press source control.

### 2.8 Video Playback Optimization

> Goal: Eliminate all stutter and delay between swipes. Videos should feel like they're already playing before the user gets there. Priority gate: do not start until basic feed video playback works end-to-end.

**Tier 1 — Highest leverage (do first):**
- [x] Pre-resolve stream URLs at ingest time: background refill job runs yt-dlp and stores `stream_url` in `feed_cache` immediately. `/api/feed/next` returns stream URLs directly in the response. Eliminates per-video `/api/stream-url` round-trips
- [x] Pre-warm feed on app load: add `feedStore.prefetch()` that fetches first batch of feed data + stream URLs during idle time on homepage (before user navigates to `/feed`). Use `requestIdleCallback` or fire after homepage render
- [x] Eager URL warm-up in store: when `fetchMore()` pulls 10 videos into buffer, immediately resolve/validate stream URLs for next 5 videos (beyond the ±2 render window). Bytes downloading before user scrolls there

**Tier 2 — Smooth out the edges:**
- [x] Video element pooling: DEFERRED — conflicts with singleton `<video>` pattern needed for iOS unmute persistence. The singleton already avoids DOM churn for the active element. Pre-resolved stream URLs (Tier 1) eliminate the main latency source. Revisit if stutter persists after mobile testing
- [x] `<link rel="preconnect">` for known CDN domains: N/A — all video loads go through same-origin `/api/proxy-stream`, so browser preconnect hints don't apply. Server-side DNS is already warm from refill jobs
- [x] Increase preload window: bump PRELOAD_AHEAD from 2 to 3-4 on fast connections. Detect via `navigator.connection.effectiveType`

**Tier 3 — Premium smoothness:**
- [ ] Service worker video segment caching: cache first ~500KB of each preloaded video response. Swipe transitions start from cache instantly while rest streams in background
- [x] Adaptive preload depth: use Network Information API to adjust strategy. 4G/WiFi → preload 4 ahead, 3G → preload 1 + lower quality (already implemented in FeedVideo._getPreloadWindow())
- [x] Stream URL TTL monitoring: proactive re-resolve for URLs expiring within 15min (5min check interval). Also fixed /api/stream-url to check expires_at before serving cached URLs

### 2.9 Mobile Feed Polish

> **Priority gate:** Do not start until 2.8 is complete (video playback is silky smooth).

- [x] Long-press timing fix: increase threshold from 500ms to ~800-1000ms so source-control sheet doesn't fire during normal scrolling or casual holds
- [x] Refresh feed button: visible button in feed UI to manually refresh/reload the feed (complement to pull-to-refresh)
- [x] Fullscreen immersive mode: button on feed overlay that hides nav bar and overlay. Video fills entire viewport, user can still scroll between videos. Tapping screen temporarily reveals overlay (auto-hides after ~3s or on next scroll)
- [x] Auto-hide nav bar: bottom nav hides on scroll down (into feed), reappears on scroll up. CSS transform slide-down/up with ~200ms transition. Reclaims screen real estate without losing access
- [x] Rethink settings access: added three-dot source control button on right side of active video (tap-friendly alternative to long-press). Settings tab already in bottom nav. Long-press still works but is no longer the only discovery path

---

## Milestone 3: Discovery & Organization

### 3.0 Source Diversification (Eliminate yt-dlp as Single Point of Failure)

> Goal: yt-dlp is unreliable and can't do discovery. Add a source adapter layer that separates discovery (finding new content) from extraction (getting stream URLs). Multiple adapters with automatic fallback so no single tool failure kills the app.

**Adapter Layer (DONE):**
- [x] `server/sources/base.js` — SourceAdapter base class with standard video shape
- [x] `server/sources/registry.js` — SourceRegistry with domain routing + fallback chains
- [x] `server/sources/ytdlp.js` — yt-dlp wrapped in adapter interface (universal fallback)
- [x] `server/sources/scraper.js` — Puppeteer discovery scraper (Pornhub, Xvideos, Spankbang)
- [x] `server/sources/cobalt.js` — Cobalt API adapter (SFW extraction fallback)
- [x] `server/sources/index.js` — Setup and registration

**Integration:**
- [x] Install puppeteer: `npm install puppeteer`
- [x] Refactor `server/index.js` to use registry instead of direct yt-dlp calls
- [x] Replace `refillCategory()` to try scraper first, fall back to yt-dlp
- [x] Replace `_refillFeedCacheImpl()` to use registry.search() with fallback
- [x] Replace `/api/metadata` to use registry.extractMetadata()
- [x] Replace `/api/stream-url` to use registry.getStreamUrl()
- [x] Replace `/api/search` SSE to use yt-dlp adapter's streamSearch (SSE pattern preserved)

**NSFW Discovery Features (new capabilities the scraper enables):**
- [x] Multi-site search: `/api/search/multi` hits all SITE_CONFIGS sites in parallel via `scraper.searchAll()`
- [x] Category browsing: `/api/categories` endpoint fetches videos from any category page URL
- [x] Trending feeds: `/api/trending` endpoint fetches per-site trending content
- [x] Add more sites to SITE_CONFIGS in scraper.js: added Redtube, YouPorn, XHamster (6 sites total)
- [x] Scheduled category/trending fetching: `startScheduledTrendingRefresh()` rotates through all NSFW sites every 30min, populates homepage_cache

**Monitoring & Resilience:**
- [x] Health check endpoint: `/api/sources/health` reports all adapter status, capabilities, versions
- [x] Per-adapter error rate tracking: successes, failures, consecutive failures, last error/success timestamps
- [x] Auto-disable adapter after 5 consecutive failures, auto re-enable after 5min cooldown, manual `/api/sources/:name/reenable`

### 3.1 Queue Sync Across Devices

> Full spec: `QUEUE_SYNC.md`

- [x] Add queue table to SQLite schema (position, video_id, added_at)
- [x] GET /api/queue endpoint: return ordered queue
- [x] POST /api/queue endpoint: add video to end (supports position for insertNext)
- [x] DELETE /api/queue/:id endpoint: remove item, reindex positions
- [x] PUT /api/queue endpoint: full reorder from ordered array of IDs
- [x] DELETE /api/queue endpoint: clear all
- [x] All mutation endpoints return full updated queue in response
- [x] Modify Zustand queue store: mutations hit API first, update from response
- [x] Remove localStorage persistence for queue (server is source of truth)
- [x] useQueueSync polling hook: fetch every 3s, pause when tab hidden, refetch on tab focus
- [x] Diff check before state update: preserves currentIndex across syncs
- [x] Offline indicator when server unreachable (OfflineBanner component)
- [x] On reconnect: server state wins (fetchQueue replaces local state)

### 3.2 Tag Preferences

- [x] Tag management UI: liked/disliked tags with add/remove in Settings page (green/red chips)
- [x] Store preferences in SQLite `tag_preferences` table
- [x] Filter library grid by selected tags (library filter tabs already include text search)
- [x] Auto-tag new videos based on yt-dlp metadata (tags field populated from yt-dlp extraction)

### 3.3 Basic Recommendations

- [x] Rule-based scoring: +2 liked tag, -5 disliked tag, +1 favorite, +1 highly rated
- [x] "Discover" endpoint: `GET /api/discover` returns unwatched videos sorted by score
- [x] GET /api/tags/popular endpoint returns top 50 tags by frequency
- [ ] Future: system searches for content discovery (see 3.4 Cookie Auth)

### 3.3.1 Seed Recommendations from PornHub History (Cookie-Powered) — HIGH PRIORITY

**Goal:** Use cookies to pull PornHub watch history/favorites via yt-dlp, extract tag/category data from those videos, and auto-populate `tag_preferences` so the recommendation engine has real signal instead of cold-starting.

**Why this matters:** The 3.3 scoring system works, but it's useless until the user manually likes/dislikes enough tags. Seeding from existing PornHub activity bootstraps the engine instantly.

**Priority:** Do this BEFORE 3.0 Integration. Cookies are imported, username is known. This is the highest-leverage feature to make FeedDeck feel personalized.

**Known info:**
- PornHub username: `tonjone92`
- Profile URL: `https://www.pornhub.com/users/tonjone92`
- Cookies installed at `data/cookies.txt` (PornHub + YouTube, both active)
- PornHub URLs to scrape with cookies:
  - `https://www.pornhub.com/users/tonjone92/videos/favorites` (favorited videos)
  - `https://www.pornhub.com/users/tonjone92/videos/watched` (watch history, if accessible)
  - `https://www.pornhub.com/users/tonjone92/videos/rated` (rated videos)
  - `https://www.pornhub.com/users/tonjone92/playlists` (user's playlists — crawl each for video metadata, highest-signal curated content)
- YouTube URLs to scrape with cookies:
  - `https://www.youtube.com/feed/history` (watch history)
  - `https://www.youtube.com/playlist?list=LL` (liked videos)
  - `https://www.youtube.com/feed/library` (saved playlists)
  - User's playlists (discover via channel page)
- TikTok username: `tmoney19060`
- TikTok URLs to scrape with cookies:
  - `https://www.tiktok.com/@tmoney19060/liked` (liked videos — primary signal)
  - `https://www.tiktok.com/@tmoney19060` (posted videos, if any)
  - Note: yt-dlp TikTok support breaks frequently. Try yt-dlp first, fall back to scraper adapter.
  - **Priority source for mobile feed** — user wants TikTok content prioritized in the swipe feed experience.
- Instagram URLs to scrape with cookies:
  - `https://www.instagram.com/<username>/saved/` (saved posts/reels)
  - Note: yt-dlp Instagram support is flaky. May need instaloader or scraper adapter as fallback.

**Backend: History Import Pipeline**
- [x] New endpoint: `GET /api/recommendations/seed` (SSE) triggers the import job
- [x] Use yt-dlp with cookies to fetch favorites/history via flat-playlist
- [x] Also try watched, rated URLs (gracefully handles private/empty with error message)
- [ ] For playlists: fetch playlist index first, then crawl each playlist for video metadata. Playlists are high-signal — curated content reveals stronger preferences than passive watch history
- [x] Parse returned JSON for each video: extract `tags`, `categories`, `uploader`, `duration`, `view_count`
- [x] Build tag frequency map from all extracted videos
- [x] Auto-insert top N tags (threshold: 2+ appearances) into `tag_preferences` as `liked`
- [x] Skip tags already in `tag_preferences` (don't override manual choices)
- [x] Import videos into `videos` table (dedup by URL) so library has content immediately
- [x] Store seed metadata: `recommendation_seed_at` timestamp + `recommendation_seed_count`

**Backend: Username Config**
- [x] Store platform usernames in `preferences` table (key: `{platform}_username`)
- [x] `PUT /api/recommendations/username` + `GET /api/recommendations/username` endpoints
- [ ] Settings UI: text field for PornHub username (pre-filled if already set)
- [x] Endpoint uses stored username to construct history/favorites URLs
- [x] Multi-platform support: pornhub, youtube, tiktok URL builders

**Frontend: Settings UI**
- [x] "Seed Now" button in Settings with platform selector + username field
- [x] Progress indicator via SSE: real-time log of scan/extract progress
- [x] Summary on completion: green card showing videosScanned, tagsFound, tagsAdded, topTags
- [x] Tags auto-added to preferences, visible in existing tag management UI above

**Edge Cases**
- [x] Handle private/empty history (graceful "not accessible" message per source)
- [x] Handle rate-limiting: 30s timeout per video, 60s per flat-playlist scan
- [x] Don't re-seed if already seeded within 24h (check `recommendation_seed_at`, `?force=1` to override)
- [x] Timeout: cap at 200 videos max

### 3.4 Cookie-Based Auth for Personalized Feeds

> Import browser cookies so yt-dlp can access logged-in content: personalized recommendations, subscriptions feed, watch history, premium content. User exports cookies via browser extension (e.g. "Get cookies.txt LOCALLY"), imports via drag-and-drop or file picker. Re-importing should be frictionless since cookies expire.

- [x] Settings UI: file picker for `cookies.txt` import with re-import and remove buttons
- [x] Cookie stored server-side at `data/cookies.txt` (ready for Pi migration)
- [x] Backend: `POST /api/cookies` receives content, validates Netscape format, writes file
- [x] yt-dlp adapter auto-detects `data/cookies.txt` and passes `--cookies` flag
- [x] Fallback to public-only content when no cookies present (no flag passed)
- [x] Cookie status indicator: green dot with count and last-modified date
- [x] `GET /api/cookies/status` and `DELETE /api/cookies` endpoints

#### 3.4.1 Per-Mode Cookie Files — NEEDS ADAPTER UPDATE

**Architecture:** Separate cookie files per mode so NSFW alt accounts don't leak into Social requests and vice versa.

**Cookie files (already created in `data/`):**
- `cookies-social.txt` — YouTube, Google, main Instagram account
- `cookies-nsfw.txt` — PornHub, NSFW Instagram alt account
- `cookies.txt` — legacy combined file (keep for backward compat until adapter is updated)

**Adapter changes needed:**
- [ ] Update `server/sources/ytdlp.js` to accept a mode parameter and pick the right cookie file: `cookies-social.txt` for social mode, `cookies-nsfw.txt` for nsfw mode
- [ ] Update all callers that pass mode context (refillCategory, feed refill, search, metadata extraction) to forward mode to the adapter
- [ ] Update `POST /api/cookies` endpoint to accept a `mode` param (social|nsfw) and write to the correct file
- [ ] Update `GET /api/cookies/status` to return status for both files
- [ ] Update Settings UI: two cookie import sections (Social cookies / NSFW cookies) with independent status indicators
- [ ] Fallback: if mode-specific file missing, try `cookies.txt` (combined), then no cookies

### 3.5 Organization Features

- [x] Favorites/heart toggle per video with filter view (heart on cards, context menu, library filter tabs)
- [x] Watch later list (toggle via context menu, library filter tab, server-synced)
- [x] Star rating (1-5) per video (star selector in context menu, optimistic local + server sync)
- [x] Custom playlists: create, add/remove, reorder (playlists + playlist_items tables, full CRUD API)

### 3.6 Search

- [x] Wire HeroCarousel search bar to `/api/search/multi` backend (multi-site parallel search)
- [x] Search across title, tags, source (library page has local text filter + SSE search)
- [x] Search results display inline in carousel strip with real thumbnails/metadata

### 3.7 Picture-in-Picture

- [x] PiP button on player + keyboard shortcut (P)
- [x] Native browser PiP API with graceful fallback (checks `document.pictureInPictureEnabled`)
- [x] While in PiP: main page shows library grid, can queue more videos (PiP is browser-native)

### 3.8 Quality Selector

- [x] Backend endpoint: `GET /api/stream-formats?url=` runs `yt-dlp -j` to list available MP4 qualities
- [x] UI: quality picker dropdown in VideoPlayer info bar (auto + available qualities)
- [x] Remember last selected quality preference in localStorage (`fd-quality`)
- [x] `GET /api/stream-url?format=` accepts optional format ID for specific quality

### 3.9 Hero Image Fitting

- [x] Hero thumbnail switched from `bg-cover` to `object-contain` with blurred scaled-up background fill behind
- [x] Radial gradient vignette overlay blends edges into background color seamlessly
- [x] Works across aspect ratios: blurred fill handles letterboxing for any ratio

### 3.10 Mobile Feed Filter System

> Depends on: 3.2 (Tag Preferences) and 3.6 (Search) for backend infrastructure. Can build the UI shell earlier but full functionality needs tags and search wired up.

- [x] Filter UI: slide-up modal or filter bar accessible from feed view
- [x] Source filter: toggle individual sources on/off to limit what appears in feed
- [x] Cross-source search: search query that runs across all active sources, results replace feed temporarily
- [x] Tag filter: filter feed by tags (requires 3.2 tag infrastructure + 3.5 auto-tagging)

### 3.11 Mobile Device Testing Gate

> **MANUAL CHECKPOINT.** Do not proceed to Milestone 4 (Pi deployment) until this is done. Prompt the user to test on a real phone over local WiFi (setup from 2.6) and sign off.

- [ ] Claude Code: prompt user to run full manual test on phone (homepage, playback, theatre mode, queue, search, swipe feed)
- [ ] User sign-off that mobile experience is acceptable before Pi migration begins

---

## Milestone 4: Deploy & Advanced

### 4.1 Raspberry Pi Deployment (now Beelink EQ12)

- [x] Production build script: `scripts/deploy.sh` — pulls, installs, builds, restarts service
- [x] Systemd service file: created in `setup-server.sh` with security hardening (NoNewPrivileges, ProtectSystem)
- [x] Setup script: `scripts/setup-server.sh` — full unattended install (Node 22, yt-dlp, npm, build, systemd, cron)
- [x] Tailscale setup: documented in `scripts/BEELINK_SETUP.md` Phase 3
- [x] Logging: `server/logger.js` — structured JSON in production (journald), colorized dev output. All server modules migrated
- [x] yt-dlp auto-update: weekly cron (Sundays 4am) in setup script
- [x] SQLite backup strategy: daily cron (3am) using `sqlite3 .backup`, 14-day retention
- [x] Bug fix: backup script pointed to wrong DB path (`server/feeddeck.db` → `data/library.db`)
- [x] Bug fix: systemd ReadWritePaths pointed to `server/` → `data/`
- [x] Added `npm start` script to package.json

### 4.2 Dual-Mode Architecture

- [x] Backend already has `mode = 'social' | 'nsfw'` in sources, categories, and feed_cache tables
- [x] Frontend modeStore already toggles between social/NSFW with panic key (Escape → social)
- [ ] Design the Social mode content pipeline (what sources? what categories?)
  > DECISION (2026-03-22): Deferred. Ship NSFW pipeline first, including 3.0 Integration.
  > NOTE: Folder rename + import path updates tracked in 1.6 Rebrand Cleanup

### 4.3 Dark/Light Theme Toggle

- [x] Tailwind dark mode class strategy: CSS custom properties for surface/text/gradient colors, `darkMode: 'class'`
- [x] Theme toggle in HomeHeader + Header: sun/moon button with `useThemeStore`
- [x] Persist preference: Zustand `persist` middleware → `fd-theme` in localStorage
- [x] Netflix-minimal dark as default: inline `<script>` in index.html prevents flash
- [x] Theme-aware gradients: HeroSection + FeaturedSection use `--color-gradient-*` variables
- [x] Theme-aware scrollbar: CSS variables for track/thumb colors

### 4.4 AI Recommendations

- [ ] Watch history + preferences → Claude API
- [ ] Taste profile scoring for new content
- [ ] "You might like this because..." UI explanations

### 4.5 Browser Extension

- [ ] Detect supported sites, inject "Save to FeedDeck" button
- [ ] One-click add to library
- [ ] Optional auto-add

### 4.6 Cross-Device Full Sync

- [ ] Sync preferences, watch history, favorites (beyond just queue)
- [ ] Simple sync code mechanism (no user accounts)

### 4.7 Offline Mode

- [ ] Download button + IndexedDB/File System Access storage
- [ ] Download progress, offline badge
- [ ] Service worker for PWA

### 4.8 In-App Source Management

- [x] Settings page at `/settings`: theme toggle, source list with pause/delete, adapter health dashboard
- [x] Source CRUD API: `GET /api/sources/list`, `POST /api/sources`, `PUT /api/sources/:domain`, `DELETE /api/sources/:domain`
- [x] Add new source: tests with registry.search() before inserting, rejects 0-result sources
- [x] Source health monitoring: adapter status badges (OK/DISABLED/UNAVAILABLE) with success/failure counts
- [x] Settings accessible from HomeHeader gear icon

---

## Milestone 5a: Video Playback (P0 — Nothing Else Matters If This Is Broken)

> Goal: Videos must play reliably across every surface — homepage theatre, feed, queue autoplay. Not just the first video. Every video.

### 5a.1 Diagnose & Fix Core Playback

- [x] Audit the full playback chain: stream URL resolution → proxy → video element → HLS.js handoff
- [x] Verify `/api/stream-url` returns valid, non-expired URLs for multiple video sources
- [x] Verify `/api/proxy-stream` correctly proxies video bytes (content-type, range requests, chunked transfer)
- [x] Check HLS.js initialization and error recovery (was silently resolving on fatal errors — FIXED)
- [x] Fix: HLS.js now attempts recovery on network/media errors, rejects on unrecoverable fatal errors
- [x] Fix: HeroSection theatre mode missing onEnded queue autoadvance — now calls advance() + resolveStream()
- [x] Fix: proxy-stream missing upstream timeout — added 15s AbortSignal.timeout()
- [x] Fix: VideoPlayer now tracks watchProgress every 5s for Continue Watching feature
- [x] Test with both direct MP4 URLs and HLS/m3u8 streams
  > All current sources return direct MP4. HLS code path verified in code (proxy rewriting + hls.js recovery). Proxy chain: fresh URL → proxy-stream → HTTP 206 video/mp4.
- [x] Confirm yt-dlp is producing working stream URLs (not geo-blocked, rate-limited, or expired)
  > 3 PornHub + 1 YouTube video all resolve. Proxy returns valid bytes for phncdn.com and googlevideo.com.

### 5a.2 Deep Playback Testing

- [?] Homepage: click a CategoryRow card → theatre mode plays video start to finish
  > API chain verified (stream-url resolves, proxy returns 206 video/mp4 bytes). Theatre mode UI activates correctly. Cannot verify actual video playback via automation — Chrome blocks media loading in MCP-controlled tabs. Needs manual browser test.
- [?] Homepage: click multiple different cards in sequence — each one plays
  > Same — needs manual verification
- [?] Feed: swipe through 5+ videos — each autoplays on snap
  > Feed API returns videos correctly. Needs manual test.
- [?] Feed: navigate away and back — playback resumes
  > Needs manual test
- [?] Queue: add 3+ videos, play through — autoadvance works, each video plays
  > Queue API works (empty queue, add/remove tested). Autoadvance code reviewed — looks correct. Needs manual test.
- [?] HeroCarousel: search results play on hover/click
  > Needs manual test
- [?] Error states: expired URL triggers re-fetch, failed video shows user-facing error (not silent black screen)
  > handleStreamError() code reviewed — retries once via resolveStream(). Needs manual test.
- [?] Edge case: rapid card switching doesn't leave zombie video elements or stale streams
  > Found 54 video elements in DOM from hover previews — potential cleanup issue. Needs manual test.

---

## Milestone 5: Design Polish & Visual Excellence

> Goal: Elevate FeedDeck's visual design beyond Netflix/YouTube/TikTok defaults. Every pixel should feel intentional. This is the "cooler than the reference apps" pass.

### 5.1 Accessibility (P0 — Ship Blockers)

- [x] Add global `focus-visible` styles: `outline: 2px solid rgba(229,9,20,0.5); outline-offset: 2px` — remove all `focus:outline-none` without replacement rings
- [x] Add `aria-label` to all icon-only buttons (~15 elements): hero "+", heart, theatre, FeaturedSection arrows, FloatingQueue chevron, CategoryRow play overlay
- [x] Add `aria-live="polite"` to FeedToast wrapper and queue count badge announcements
- [x] Add screen reader announcement for mode switch (SFW/NSFW)
- [x] Fix color contrast: bump text-muted from #6b6b70 to #8a8a90 (4.5:1). Only use accent (#e50914) at 18px+ bold or bump to #ff2d36 for small text
- [x] Add `<main>` landmark wrapper to page content areas
- [x] Add skip-nav link for keyboard users

### 5.2 Navigation Unification (P1)

- [x] Extend HomeHeader nav pattern (Home | Feed | Library) to work on all pages
- [x] Remove dead routes from FeedBottomNav (/feed/search, /feed/settings don't exist)
- [x] On mobile: single bottom tab bar that works across all views (not just feed)
- [x] Active state indicator: accent underline or dot on current nav item

### 5.3 Typography Overhaul (P1 — Highest Visual Impact)

> The single biggest identity upgrade. DM Sans is forgettable. This gives FeedDeck a recognizable voice.

- [x] Replace DM Sans with dual-font system: **Space Grotesk** (display/headlines) + **Inter** (UI/body)
- [x] Update Tailwind `fontFamily`: `sans: ['Inter', ...]` and add `display: ['"Space Grotesk"', ...]`
- [x] Apply `font-display` to hero title, FeaturedCard title, section headers, logo
- [x] Fix typography hierarchy collision: FeaturedSection header and card title both 22px/bold
  - Featured header → 14px/600, uppercase, tracking-wider (label style)
  - Featured card title → 26px/700, -0.5px tracking (hero style)
  - Category headers → 18px/700, -0.3px tracking (up from 15px)
  - Category card title → 13px/600 (up from 12px)
- [x] Hero carousel card text too small: bump title from 10px to 11px/600, duration from 9px to 10px/500
- [ ] Logo treatment: replace emoji (📡) with SVG mark, drop italic. Consider `font-family: 'Geist Mono'` at 15px/700 for technical feel
  > DECISION (2026-03-22): Emoji is fine for now. Typography overhaul is higher impact. Revisit post-launch.

### 5.4 Color & Accent Identity (P1)

> Stop looking like a Netflix clone. Own a color.

- [x] Shift accent from Netflix Red (#e50914) to Rose (#f43f5e) — one Tailwind token change
- [x] Update accent-hover and accent-muted tokens to match new accent
- [x] Warm up base surface: shift #0a0a0b to #111113 (subtle blue tint)
- [x] Consolidate raw color values to tokens → moved to Milestone 6 (6.1.3)
- [x] Establish two glass material tokens in index.css → moved to Milestone 6 (6.1.2)
- [x] Add 1px top highlight on raised cards for depth → moved to Milestone 6 (6.1.4)
- [x] Mode toggle pill: reduce SFW amber from `bg-amber-500/[0.12]` to `bg-amber-500/[0.06]` — too bright, competes with hero CTA

### 5.5 Motion & Micro-Interactions (P1)

> Make every interaction feel intentional.

- [x] Unify card hover pattern across all card types: `hover:scale-[1.03] hover:-translate-y-0.5 hover:shadow-[0_12px_32px_rgba(0,0,0,0.4)] transition-all duration-200 ease-out`
  - Currently: VideoCard scale-105, CategoryRow scale-105/-translate-y-1, CarouselCard scale-[1.06]/-translate-y-1 (three different behaviors)
- [x] Add `active:scale-[0.97]` pressed states to all interactive elements globally
  - Feed buttons currently at `active:scale-90` (too aggressive) → soften to `active:scale-95`
- [x] Add page transition animation between routes → moved to Milestone 6 (6.3.1)
- [x] Queue pulse animation: extend from 0.2s to 0.35s with spring easing `cubic-bezier(0.34, 1.56, 0.64, 1)` + glow ring `box-shadow: 0 0 0 4px rgba(accent, 0.3)`
- [x] Hero scroll affordance → moved to Milestone 6 (6.3.5)
- [x] Tighten FeaturedSection scroll zone → moved to Milestone 6 (6.3.6)

### 5.6 Spacing & Layout Rhythm (P1)

- [x] Establish single page gutter token: px-10 for homepage, px-4/md:px-6 for dense pages (intentional density difference)
- [x] Unify card gaps: `gap-3` (12px) for horizontal scroll rows, `gap-4` (16px) for grids
- [x] FeaturedSection cards: switch from fixed 380px height to `height: clamp(320px, 28vw, 420px)` for responsive scaling
- [x] Hero content positioning → moved to Milestone 6 (6.2.6)

### 5.7 Component-Level Polish (P2)

- [x] HomeHeader nav: add 2px bottom accent indicator on active nav item (already done in 5.2)
- [x] ContextMenu star rating: move "Rate" label above stars, increase tap target to `w-7 h-7`, add `hover:text-amber-300`
- [x] FloatingQueue vs. mobile preview button conflict: gated preview toggle behind `import.meta.env.DEV`
- [x] SourceControlSheet: add glassmorphism (`backdrop-blur-2xl`) + top glow shadow (`shadow-[0_-4px_24px_rgba(0,0,0,0.3)]`)
- [x] Hero carousel gradient: lighten from `from-black/[0.85]` to `from-black/[0.75]` so text isn't buried

### 5.8 Empty States & Copy (P2)

- [x] Feed empty state: replace "Check back once the backend has fetched content" with actionable CTA ("Add your first source to start discovering videos")
- [x] Library empty state: "Start building your library" with Browse Feed CTA
- [x] Queue empty state: already decent but could use illustration or visual
- [x] All empty states should have a primary action button

### 5.9 Library Page Upgrade (P2)

- [x] Build out Library as first-class page matching homepage visual language
- [x] Tabs: All, Favorites, Watch History, Watch Later, Top Rated (with count badges)
- [x] Continue Watching row at top with resume progress indicators
- [x] Add watchProgress tracking to libraryStore (0-1 fraction, setWatchProgress action)
- [x] Per-tab contextual empty states with actionable CTAs
- [x] Make Library reachable from all views via unified nav (done in 5.2)

### 5.10 Bundle & Performance

- [x] Add code-splitting to reduce 789KB JS bundle (Vite build warning)
- [x] Add eslint.config.js for ESLint v9+ (currently no config, lint check fails)
- [x] modeStore.js mixed dynamic/static import warning (Vite build)

---

## Milestone 6: Visual Conviction (Apple TV / YouTube Parity)

> Goal: Close the gap between "developer-built app" and "designed product." FeedDeck has the structure; this milestone adds conviction. Every surface, every hover, every transition should feel like someone decided it should be exactly that way. Audit ref: `MORNING_SPRINT_2026-03-24.md` design-polish deep audit.

### 6.1 Icon System & Material Language (Tier 1 — Highest visual impact for effort)

- [ ] Replace all unicode/emoji icons with Lucide React across the entire app
  - `HomeHeader.jsx`: ⚙ → `Settings`, ☀ → `Sun`, 🌙 → `Moon`, ● → `Circle` (mode indicator)
  - `VideoCard.jsx`: + → `Plus`, ▶ → `Play`, ♡/♥ → `Heart`, ✕ → `X`
  - `CategoryRow.jsx`: ▶ play overlay → `Play`
  - `FeaturedSection.jsx`: ‹/› arrows → `ChevronLeft`/`ChevronRight`, + → `Plus`, ▶ → `Play`
  - `QueueSidebar.jsx`: 📋 → `ListVideo`, ▶ → `Play`, ✕ → `X`
  - `FloatingQueue.jsx`: queue icon → `ListVideo`
  - `LibraryPage.jsx`: empty state icons (📂 → `FolderOpen`, ♡ → `Heart`, ⏱ → `Clock`, 🔖 → `Bookmark`, ★ → `Star`)
  - `TheatreControls.jsx`: all player icons → Lucide equivalents
  - `FeedPage.jsx` / feed components: all overlay icons
  - `SettingsPage.jsx`: section icons if any
  - Logo: keep 📡 for now (per 5.3 decision), but wrap in `<span>` for future SVG swap
- [ ] Verify Lucide is installed (`npm ls lucide-react`) — install if missing
- [ ] Establish two glass material tokens in `index.css` and matching Tailwind utility classes:
  - `--glass-thin`: `backdrop-blur-[12px] bg-[rgba(20,20,22,0.6)] border border-white/[0.08]`
  - `--glass-thick`: `backdrop-blur-[20px] bg-[rgba(20,20,22,0.8)] border border-white/[0.12]`
  - Light mode equivalents: thin `bg-[rgba(248,248,250,0.7)]`, thick `bg-[rgba(248,248,250,0.85)]`
- [ ] Apply glass-thin to: `HomeHeader`, `FeaturedSection` nav arrows, `FeedFilterSheet`, hover overlay badges, duration badges
- [ ] Apply glass-thick to: `TheatreControls`, `FloatingQueue` expanded panel, `SourceControlSheet`, `ContextMenu`, `AddVideoModal`
- [ ] Consolidate raw color values to design tokens: audit and replace all `bg-gray-*`, `bg-white/*`, `bg-black/*`, `rgba(...)` inline values with surface/text/accent token equivalents. Target: zero raw color values outside `index.css`
- [ ] Add 1px top highlight (`border-t border-white/[0.04]`) on all raised cards for depth:
  - `CategoryRow` cards, `VideoCard`, `ContinueWatchingCard`, `FeaturedCard` (in carousel state), `QueueSidebar` items on hover
  - Light mode: `border-t border-black/[0.04]`
- [ ] Unify border-radius to two values: `rounded-lg` (8px) for cards/containers, `rounded-full` for pills/avatars/buttons. Kill `rounded-[10px]`, `rounded-2xl`, `rounded-xl` inconsistencies
  - `CategoryRow` cards: `rounded-[10px]` → `rounded-lg`
  - `ContinueWatchingCard`: `rounded-[10px]` → `rounded-lg`
  - `ContextMenu`: verify `rounded-lg`
  - `SettingsPage` sections: verify `rounded-xl` → `rounded-lg`

### 6.2 Typography & Spacing System (Tier 2 — High impact, more work)

- [ ] Consolidate type scale to 6 defined sizes — kill all arbitrary `text-[Npx]` values:
  - `text-2xs` (10px) — badges, timestamps only
  - `text-xs` (11px) — metadata, secondary labels
  - `text-sm` (13px) — body, card titles, UI controls
  - `text-base` (15px) — primary body, descriptions
  - `text-lg` (18px) — section headers
  - `text-xl` (24px) — page titles
  - `text-hero` (clamp(28px, 4vw, 48px)) — hero only
  - Add these to `tailwind.config.js` `fontSize` extend
- [ ] Consolidate tracking to 3 values:
  - `tracking-tight` (-0.3px) — display/headlines (Space Grotesk)
  - `tracking-normal` (0) — body text (Inter)
  - `tracking-wide` (0.8px) — uppercase labels, badges
  - Kill: `tracking-[1.4px]`, `tracking-wider`, `tracking-[-0.5px]`, all one-off values
- [ ] Consolidate opacity values to a defined set of 5:
  - `opacity-0` — hidden
  - `opacity-40` — disabled / deeply muted
  - `opacity-60` — secondary / inactive
  - `opacity-80` — soft emphasis
  - `opacity-100` — full
  - Also standardize `white/8`, `white/12`, `white/20` as the three transparency levels (kill `white/10`, `white/15`, `white/16`, `white/[0.08]` etc.)
- [ ] Establish spacing rhythm on 8px base grid. Audit and fix:
  - Card internal padding: standardize to `p-3` (12px) everywhere (currently `p-2.5`, `p-2.5 pt-2`, varies)
  - Section margins: `mb-8` (32px) between category rows (currently `mb-9` = 36px, not on grid)
  - Section header to content: `mb-4` (16px) consistently
  - Page gutter: `px-10` (40px = 5x8) already on grid, keep
  - Card gaps: `gap-3` (12px) horizontal rows, `gap-4` (16px) grids — already correct
- [ ] Deepen hover shadows with dual-shadow for physical realism:
  - Cards: `hover:shadow-[0_8px_24px_rgba(0,0,0,0.25),0_2px_6px_rgba(0,0,0,0.15)]`
  - Replace current single `hover:shadow-[0_12px_32px_rgba(0,0,0,0.4)]` (too dark, no spread variation)
  - Add shadow transition: `transition-shadow duration-200`
- [ ] Hero content positioning: replace `bottom-[230px]` pixel magic with flexbox `justify-end pb-[max(180px,18vh)]` so hero text adapts to viewport height. Affects: `HeroSection.jsx`, `SkeletonHero`

### 6.3 Motion & Route Polish (Tier 3 — Final refinement)

- [ ] Add route transition animation: 150ms opacity crossfade using CSS View Transitions API (Chrome native). Fallback for non-supporting browsers: plain CSS opacity transition on `<main>` with `useLocation` key change
  - No Framer Motion dependency — CSS only per 5.5 decision
- [ ] Upgrade skeleton shimmer from `animate-pulse` (opacity breathe) to gradient sweep:
  - `@keyframes shimmer { 0% { background-position: -200% 0 } 100% { background-position: 200% 0 } }`
  - `background: linear-gradient(90deg, transparent 25%, rgba(255,255,255,0.04) 50%, transparent 75%)`
  - `background-size: 200% 100%`
  - `animation: shimmer 1.5s ease-in-out infinite`
  - Apply to all skeleton components in `Skeletons.jsx`
- [ ] Add universal `active:scale-[0.97] active:brightness-90` to all buttons that don't already have active states. Verify `TheatreControls`, `HomeHeader`, `LibraryPage` tab buttons, `SettingsPage` action buttons
- [ ] CategoryRow horizontal scroll: add keyboard navigation (`ArrowLeft`/`ArrowRight` when row is focused) and `role="list"` / `role="listitem"` for screen readers
- [ ] Hero scroll affordance: reduce hero from 100vh to ~85vh so FeaturedSection content peeks above fold. Self-documenting scroll UX without a chevron
- [ ] Tighten FeaturedSection scroll zone from 550vh to ~300vh:
  - Retune phase breakpoints in `useFeaturedScroll.js` proportionally
  - Show thin progress bar during scroll (already exists, verify visibility)
  - Add "Skip" text link at bottom of sticky container that scrolls past the zone
- [ ] Focus indicator upgrade: increase focus-visible ring from `rgba(244, 63, 94, 0.5)` to `rgba(244, 63, 94, 0.7)` for 3:1 contrast ratio against dark surfaces. Add `outline-width: 2.5px`
- [ ] Audit and fix `prefers-color-scheme` auto-detection: if user hasn't manually toggled theme, default to system preference. Currently always defaults to dark

### 6.4 Feed Experience Overhaul (TikTok/Reels Parity)

> Goal: Transform the feed from a functional video list into an addictive, minimal, personalized experience. Every scroll should feel physical. Every interaction should give instant feedback. The feed should never show a black screen, never dead-end, and never make you think about settings.

**6.4.1 Kill the Black Frame (P0 — Most Critical UX Bug)**
- [ ] Keep thumbnail visible during video load: `FeedVideo.jsx` currently hides thumbnail when `isActive` is true (`!isActive && video.thumbnail`). Change to: always render thumbnail, layer video on top, only hide thumbnail after video `onPlaying` fires
- [ ] Add crossfade: thumbnail at `opacity-100`, video starts at `opacity-0`, transitions to `opacity-100` on `playing` event (200ms ease-out). User never sees black
- [ ] Add blurred thumbnail background: behind the main thumbnail, render a `blur-2xl scale-110` version as ambient fill. Eliminates black bars on mismatched aspect ratios
- [ ] Preload next video's thumbnail image: when `currentIndex` changes, trigger `new Image().src = buffer[currentIndex + 1]?.thumbnail` so the next slide's thumbnail is already cached

**6.4.2 Engagement Sidebar (P0 — Core Missing Feature)**
- [ ] Add TikTok-style right-side action column to `FeedVideo.jsx` when `isActive`. Vertical stack, `bottom-24 right-3`, `gap-5`:
  - **Heart** button: Lucide `Heart` icon (24px), fill on active, pulse animation on tap. Wired to `toggleFavorite(video.id)` from libraryStore
  - **Add to queue** button: Lucide `ListPlus` icon (24px). Shows `QueueSwipeAnimation` on tap
  - **Share/Copy link** button: Lucide `Share2` icon (22px). Copies `video.url` to clipboard, shows toast "Link copied"
  - **Source info** button: Lucide `MoreVertical` icon (22px). Opens `SourceControlSheet`
  - All buttons: `w-11 h-11 rounded-full bg-black/30 backdrop-blur-sm`, white icons, with count labels below (like count, queue position)
- [ ] Show like count below heart button if available from video metadata (`video.view_count` or `video.like_count`). Format as compact: 1.2K, 45K, 1.2M
- [ ] Remove the lone three-dot button currently at `bottom-20 right-3` (replaced by sidebar)
- [ ] Circular progress ring around one of the sidebar buttons (like TikTok's sound icon) showing video playback progress as alternative to bottom timeline bar

**6.4.3 Separate Tap Actions (P0 — Usability Fix)**
- [ ] Decouple mute from pause. New gesture model:
  - **Single tap** (center area): play/pause toggle. Always. Show animated pause/play icon (Lucide `Pause`/`Play`, scale up from 0 to 1 then fade out over 500ms, centered on screen)
  - **Tap mute indicator** (top-right icon): unmute. Direct, not hidden behind a tap-anywhere gesture
  - **Double tap** (anywhere): heart/like (keep current behavior)
  - **Long press**: source controls (re-enable in `useFeedGestures.js`, was disabled)
- [ ] Add visible mute/unmute toggle button to the engagement sidebar or top-right. Lucide `Volume2`/`VolumeX`, same glass pill style as filter button
- [ ] Remove the conflated unmute-then-pause tap logic from `FeedVideo.handleTap`

**6.4.4 Desktop Feed Layout (P1 — Desktop feels wrong)**
- [ ] On desktop (`min-width: 768px`), constrain feed to phone-width column: `max-w-[480px] mx-auto` with dark background fill
- [ ] Add side panels flanking the feed column:
  - **Left panel**: video title, description, tags, uploader info (moved from overlay to sidebar). Avoids text-over-video readability issues
  - **Right panel**: "Up next" preview (next 3 thumbnails in buffer), queue summary, keyboard shortcut hints
- [ ] On mobile, keep current full-width behavior
- [ ] Desktop keyboard shortcuts overlay: first visit shows a dismissible tooltip with J/K/L/Arrow shortcuts

**6.4.5 Swipe Feedback (P1 — Gestures Feel Dead)**
- [ ] Add horizontal swipe tracking to `useFeedGestures.js`: expose `swipeOffset` state that updates during `touchmove` with the horizontal delta (capped at +/- 120px)
- [ ] In `FeedVideo.jsx`, apply `transform: translateX(${swipeOffset}px)` and `rotate(${swipeOffset * 0.02}deg)` to the video container during horizontal swipe. Creates the card-drag effect
- [ ] Add directional indicator during swipe: left swipe reveals accent-colored `ListPlus` icon (queue), right swipe reveals `ExternalLink` icon (open source). Icons fade in proportionally to swipe distance
- [ ] On swipe commit: card slides out fully with spring easing (`cubic-bezier(0.34, 1.56, 0.64, 1)`, 300ms), then next video snaps in
- [ ] On swipe cancel (below threshold): card springs back to center with overshoot

**6.4.6 Video Duration Context (P1 — Users Need to Know What They're Watching)**
- [ ] Add duration badge on the video overlay: `FeedVideo.jsx`, top-left, glass pill style, shows `video.duration` formatted (e.g., "2:34" or "12:05"). Appears for 3s on video enter, then fades. Tapping overlay re-shows it
- [ ] Replace thin timeline bar with a more visible scrubber: increase height from 3px to 4px, add expand-on-hover to 8px (desktop only), allow tap-to-seek by calculating `(clientX / width) * duration` and setting `currentTime`
- [ ] Add elapsed/remaining time on timeline hover (desktop): small tooltip showing "1:23 / 4:56"
- [ ] Auto-scrolling text ticker for long titles: if title exceeds 2 lines, marquee-scroll the text horizontally (CSS `animation: marquee 8s linear infinite` after 2s delay)

**6.4.7 Never Dead-End (P1 — Feed Should Be Infinite)**
- [ ] Replace "You're all caught up" with an automatic strategy pivot: when `exhausted` is true, fetch from `/api/discover` (random/trending) instead of `/api/feed/next`. Show a subtle "Exploring new content" toast so user knows the source changed
- [ ] Add "Not interested" swipe-down gesture on the exhausted screen to force-refresh from a different source/tag
- [ ] If discover also returns empty, show a minimal "Add more sources" CTA with a link to settings, but never a dead screen with just a checkmark

**6.4.8 Scroll Feel & Transitions (P2 — Polish)**
- [ ] Add overscroll resistance at first video: CSS `overscroll-behavior-y: contain` on the container, plus a visual pull indicator (small downward arrow that appears and stretches proportionally to overscroll distance, triggers refresh on release)
- [ ] Video enter micro-animation: when a new video snaps into view, apply a subtle `scale(1.02) -> scale(1)` + `opacity(0.9) -> opacity(1)` transition over 200ms to the thumbnail/video. Creates an "arrival" moment
- [ ] Feed loading state upgrade: replace spinner with skeleton video card (blurred gradient + shimmer) that matches the feed slot dimensions

**6.4.9 Implicit Personalization (P2 — Replace Explicit Filters)**
- [ ] Track watch duration per video in `feedStore`: if user watches >75% of a video, auto-boost that source/tag. If user skips within 2s, auto-demote. Send to `/api/feed/implicit-feedback` with `{ videoId, watchPercent, action: 'boost'|'skip' }`
- [ ] Backend: weight implicit signals into feed ranking (lighter than explicit tag preferences, but additive)
- [ ] Reduce filter sheet to a secondary "advanced" option. Primary feed curation should happen through behavior, not settings
- [ ] Add "Why this video?" tooltip (long-press on title or info icon): shows which tags/sources caused this video to appear. Transparency builds trust

**6.4.10 Feed Onboarding (P2 — First Run Experience)**
- [ ] First-time feed visitors (no watch history): show 3-card swipe tutorial overlay explaining gestures:
  - Card 1: "Swipe up/down to browse" (vertical arrow animation)
  - Card 2: "Double tap to like" (heart burst preview)
  - Card 3: "Swipe left to queue" (card-slide animation)
- [ ] Tutorial auto-dismisses after the first vertical swipe, stores `fd-feed-onboarded` in localStorage
- [ ] For users with no sources configured: skip tutorial, go straight to a "Pick your interests" tag cloud (top 20 tags from all available sources) that seeds `tag_preferences` before first feed load

### 6.5 Homepage Overhaul (Streaming Showcase Parity)

> Goal: The homepage should sell content like Apple TV / Netflix — hero that doesn't trap you, categories that feel personal, seamless theatre mode, instant content discovery. No faith-scrolling, no dead space, no generic shelves.

**6.5.1 Hero Viewport Fix (P0 — Content Discovery Blocker)**
- [ ] Reduce hero from 100vh to ~70vh so CategoryRow content peeks above the fold. Users should see content exists without scrolling. Affects `HeroSection.jsx` and `SkeletonHero`
- [ ] Add a subtle scroll indicator at the bottom of the hero (thin animated line or faded content preview, not a chevron) that self-documents scrollability
- [ ] Add Continue Watching row as the first row below the hero (above FeaturedSection). Pull from `watch_history` table, filter >5% and <90% completion. This is the highest-engagement row on Netflix/Disney+ and it's missing from the homepage

**6.5.2 Theatre Mode Polish (P0 — Playback UX)**
- [ ] Add visible exit button to theatre mode: Lucide `X` icon, top-right, glass-thin pill style, shows on hover/tap. Currently no visual affordance to exit — requires knowing Escape key
- [ ] Auto-exit theatre mode on scroll: if user scrolls down while in theatre mode, exit smoothly and reveal CategoryRows. Current behavior traps users in theatre mode
- [ ] Theatre mode video end behavior: on video end, show "Next up" card with 5s countdown auto-advance (like Netflix post-play), or auto-advance if queue has items. Currently just stops

**6.5.3 FeaturedSection Scroll Zone (P1 — Engagement Risk)**
- [ ] Make FeaturedSection cards tappable during scroll animation (currently non-interactive until REVEALED state). Clicking a card during any phase should navigate to it or start playback
- [ ] Add "Skip" affordance: small text link or tap target that scrolls past the 550vh zone. Power users who've seen the animation 10 times need an escape hatch
- [ ] Consider reducing scroll zone from 550vh to ~300vh (see 6.3 task) and making phase transitions snappier. The animation is impressive but the length taxes patience

**6.5.4 Search Relocation (P1 — Discoverability)**
- [ ] Move search from HeroCarousel (where it's buried) to a global search accessible from HomeHeader. Lucide `Search` icon in the header, expands to full-width search bar with `backdrop-blur` overlay
- [ ] Keep multi-site search functionality and 380ms debounce, just surface it at the top level where users expect it
- [ ] Add recent searches and trending tags to the expanded search dropdown

**6.5.5 CategoryRow Improvements (P1 — Content Browsing)**
- [ ] Remove staggered card entrance animations (currently `index * 60ms` delay per card = 600ms for 10 cards). Show rows instantly on scroll. The animation reads as loading delay, not polish
- [ ] Add "Because you liked [tag]" and "Your top sources" personalized rows using tag_preferences and watch history data. At minimum 1 personalized row in the top 3 positions
- [ ] Sparse row handling: if a category has <4 items, merge into a "Mixed" row or hide until it has enough content. A 5-column grid with 2 items looks broken
- [ ] Add "See all" link at the end of each row that navigates to LibraryPage with that category pre-filtered

**6.5.6 HomeHeader Active State (P2 — Navigation Clarity)**
- [ ] Improve active route indicator: add accent-colored underline or pill background behind the active nav item (currently only opacity difference: `text-white` vs `text-white/60`, nearly invisible)
- [ ] Animate route indicator on navigation: pill/underline slides to the new active item with a 200ms ease transition
- [ ] Add scroll-based header opacity: header starts fully transparent on hero, gains glass-thin background as user scrolls past hero section

**6.5.7 Homepage Loading States (P2 — Perceived Performance)**
- [ ] Add skeleton-to-content transitions per CategoryRow: each row independently shows skeleton then fades to real content as data arrives. Currently either all-skeleton or all-content
- [ ] Hero skeleton should show a blurred gradient + shimmer (matching 6.3 shimmer upgrade) instead of the current flat placeholder
- [ ] Prefetch first 2 CategoryRows on route enter so above-fold content loads instantly, lazy-load remaining rows on scroll

### 6.6 Library Overhaul (Personal Collection UX)

> Goal: Library should feel like your curated collection — easy to browse, easy to find, easy to manage. YouTube Library meets Spotify "Your Library" with better empty states and mobile-ready interaction patterns.

**6.6.1 Tab UX Upgrade (P1 — Mobile & Interaction)**
- [ ] Replace text tab bar with horizontally-scrollable pill/chip row (YouTube-style). Each pill: `rounded-full px-4 py-2 bg-surface-2`, active pill gets `bg-accent text-black`. Scrolls on mobile, wraps on desktop
- [ ] Add sort control within each tab: dropdown or toggle for "Recently added", "Most watched", "A-Z", "Duration". Default: "Recently added" for all tabs except Watch History (which should default to "Recently watched")
- [ ] Add search bar within library: sticky below the tab row, filters current tab results with 300ms debounce. Lucide `Search` icon + input field
- [ ] Continue Watching should be a dedicated row above the tab bar (like Netflix), not a tab. It's the highest-priority content and shouldn't be buried in tab navigation

**6.6.2 Empty State Overhaul (P1 — Onboarding)**
- [ ] Upgrade empty states from generic "Start browsing" CTAs to contextual actions:
  - **Favorites empty**: "Heart videos in the feed or library to see them here" + CTA "Go to Feed"
  - **Watch History empty**: "Videos you watch will appear here" (no CTA needed — it populates automatically)
  - **Watch Later empty**: "Swipe left on any video in the feed to save it for later" + CTA "Go to Feed"
  - **Top Rated empty**: "Rate videos to build your top list" (shows when no ratings exist)
- [ ] Add illustration or animation to empty states instead of just an icon + text. A simple SVG animation (e.g., a card dropping into a collection) adds warmth
- [ ] For "All" tab empty: if no sources are configured, show a prominent "Add your first source" card that links directly to Settings > Feed Sources

**6.6.3 Continue Watching Intelligence (P1 — Content Management)**
- [ ] Auto-remove videos at >90% completion from Continue Watching (Netflix behavior). Mark as "watched" in history instead
- [ ] Add "Remove from Continue Watching" swipe action or X button per card. Users need to manually clean up without clearing all history
- [ ] Show progress bar accuracy: currently tracks every 5s. Increase to every 2s for the last 20% of the video so the bar is accurate near completion
- [ ] Sort Continue Watching by most recently watched, not by date added

**6.6.4 Touch-Friendly Actions (P1 — Mobile)**
- [ ] Add long-press context menu for VideoCard on touch devices (mobile equivalent of right-click context menu). Options: Play, Add to Queue, Favorite, Copy Link, Remove from Library
- [ ] Add swipe-to-delete on library cards (horizontal swipe reveals red "Remove" button, like iOS Mail)
- [ ] VideoCard thumbnail should show play button overlay on mobile (currently only shows on desktop hover)

**6.6.5 Grid & Layout Polish (P2 — Visual Quality)**
- [ ] Dynamic column count based on content: if a tab has <6 items, use fewer columns and center the grid. Prevents the "3 cards in a 5-column grid" dead space issue
- [ ] Add grid/list view toggle: grid for browsing, list for scanning (shows title + duration + source in a compact row). Persist preference in localStorage
- [ ] Add "Recently Added" badge on videos added in the last 24h: small accent dot or "NEW" label on the thumbnail corner

**6.6.6 Watch History Management (P2 — Housekeeping)**
- [ ] Add "Clear all history" button with confirmation dialog at the top of Watch History tab
- [ ] Add "Clear last hour" and "Clear last 24h" as quick options alongside "Clear all"
- [ ] Add per-item remove: X button or swipe-to-remove on each history entry
- [ ] Show watch date/time on history items: "Watched 2h ago", "Yesterday at 11pm"

### 6.7 Settings Overhaul (Consumer-Grade Control Panel)

> Goal: Settings should be scannable, grouped logically, and never overwhelming. iOS Settings meets YouTube Settings — clean sections, progressive disclosure, no developer tooling exposed to end users.

**6.7.1 Consistent Shell (P0 — Navigation Coherence)**
- [ ] Replace SettingsPage's custom header with the shared `HomeHeader` component. Currently uses a different sticky header with built-in nav, creating a jarring transition from other pages. Settings should live inside the same AppShell as everything else
- [ ] Add Settings as a route within the existing nav, or use a slide-out panel from the gear icon. The current full-page navigation break feels like leaving the app

**6.7.2 Section Grouping & Progressive Disclosure (P1 — Information Architecture)**
- [ ] Group 6 sections into 2-3 collapsible categories:
  - **Preferences**: Appearance + Tag Preferences (user-facing controls)
  - **Sources & Content**: Feed Sources + Browser Cookies + Seed Recommendations (content pipeline)
  - **System**: Adapter Health (advanced/debug)
- [ ] Collapse sections by default, expand on click. Only "Preferences" expanded on first load. Reduces cognitive load from 6 visible sections to 2-3 category headers
- [ ] Add section anchors: clicking a section in a sticky sidebar (desktop) or top pill nav (mobile) scrolls to that section

**6.7.3 Seed Progress UX (P1 — Replace Dev Tooling)**
- [ ] Replace raw SSE terminal log with a clean progress UI:
  - Progress bar (determinate when page count is known, indeterminate otherwise)
  - Current status as a single line: "Discovering content from PornHub... (page 3 of 5)"
  - Completion: "Found 47 new videos" with a "View in Library" CTA
  - Error: "Failed to reach [source]" with retry button
- [ ] Move the raw log behind a "Show details" toggle for power users who want the terminal output
- [ ] Add estimated time remaining based on historical seed duration per source

**6.7.4 Source Management UX (P1 — CRUD Polish)**
- [ ] Replace inline add-source form with a modal dialog: prevents layout shifts, gives the form breathing room, and allows a cancel button without collapsing state
- [ ] Add source validation feedback: after entering a URL, show "Checking..." then either a green checkmark with source name/type detected, or a red X with error reason
- [ ] Add source card redesign: each source shown as a card with favicon, name, status indicator (green dot = active, yellow = paused, red = error), video count, last fetched timestamp
- [ ] Batch actions for sources: select multiple + pause/resume/delete

**6.7.5 Tag Preferences Feedback (P1 — Interaction Confidence)**
- [ ] Add save confirmation on tag preference changes: brief toast "Preferences updated" or inline checkmark animation next to the changed tag
- [ ] Show preview of how tag changes affect content: "Liking 'amateur' will boost ~12 videos in your feed" (query the backend for a count)
- [ ] Group tags by category or source rather than flat alphabetical list. Collapsible groups make long tag lists manageable

**6.7.6 Cookie Import Validation (P2 — Error Prevention)**
- [ ] Add cookie paste validation: parse the input on paste, show "Valid: 47 cookies from 3 domains" or "Error: Invalid Netscape cookie format — expected tab-separated fields"
- [ ] Add per-domain cookie count breakdown after import: "PornHub: 23, YouTube: 18, Other: 6"
- [ ] Add "Test cookies" button that makes a lightweight request to each domain and reports if the cookies are still valid/logged in

**6.7.7 Debug Panel Gating (P2 — Clean Consumer UX)**
- [ ] Move Adapter Health behind a "Developer" or "Advanced" section that's hidden by default. Reveal with a toggle or a 5-tap Easter egg on the Settings header
- [ ] Replace raw adapter status with a simplified health summary: green "All sources working" or yellow "1 source needs attention" at the top of the Sources section. Only show the full debug panel when the user drills in
- [ ] Add automatic health monitoring: if an adapter fails 3 times, surface a notification on the homepage ("Source X isn't responding — check settings") instead of requiring users to check Settings proactively

**6.7.8 Destructive Action Safety (P2 — Error Prevention)**
- [ ] Audit all destructive actions and add confirmation dialogs where missing:
  - Delete source: "Remove [source name]? This won't delete downloaded content" — Confirm / Cancel
  - Clear cookies: "Clear all browser cookies? You may need to re-import them" — Confirm / Cancel
  - Reset tag preferences: "Reset all tag preferences to neutral?" — Confirm / Cancel
- [ ] Add undo support for deletions where feasible: "Source removed. Undo (5s)" toast with countdown. Soft-delete in DB, hard-delete after timeout

### 6.8 Video Detail Page (Watch Page)

> Goal: Clicking play on any video should navigate to a dedicated watch page where the video auto-plays immediately. No double-click. No loading into the hero. This is the single most important UX flow in the app and it currently doesn't exist.
>
> **Design brief:** YouTube-meets-Letterboxd. Personality 6/10. Content-first. Differentiator: transparent related video reasoning ("More from [creator]", "Because you watch [tag]").
> **Layout:** Option A (YouTube Evolved) — asymmetric 2-column desktop, single-column mobile. Player + info on left (65%), related sidebar on right (35%).
> **Wireframe:** `watch-page-wireframe.html` in project root. Includes 3 layout options + mobile frame + design notes.

**6.8.1 Route & Page Shell (P0 — Core Missing Page)**
- [ ] Create `/watch/:id` route in `AppShell.jsx`. Lazy-loaded like other pages. Receives video ID as param, resolves from library DB or fetches metadata on demand if not in library
- [ ] Create `WatchPage.jsx` page component:
  - Shared `HomeHeader` at top (consistent navigation — no custom header like SettingsPage)
  - Desktop: `grid grid-cols-[1fr_380px] gap-6` inside `max-w-[1400px] mx-auto px-6`
  - Mobile (<768px): single column, no sidebar, related stacks below info
  - Sidebar: `position: sticky; top: 80px; max-height: calc(100vh - 100px); overflow-y: auto`
- [ ] Wire ALL "play" actions across the app to `navigate('/watch/${video.id}')`:
  - `VideoCard.jsx` play button and click handler
  - `CategoryRow.jsx` card clicks
  - `LibraryPage.jsx` card clicks and Continue Watching clicks
  - `HeroCarousel.jsx` card clicks
  - `FeaturedSection.jsx` card clicks (in REVEALED state)
  - Queue "now playing" click
  - Search result clicks
- [ ] Auto-play on mount: video begins playing as soon as the page loads. No play button gate. If browser blocks autoplay (common on first visit), show a centered play button overlay: Lucide `Play` icon (28px) inside a 72px circle, `bg-accent`, `hover:scale-[1.08]`. Single tap starts playback
- [ ] SFW mode: shows puppy video detail page with puppy-themed related content
- [ ] Page enter animation: staggered fade-up on mount. Player container (0ms delay), info section (80ms), sidebar (160ms). `opacity: 0 → 1, translateY: 8px → 0, 250ms ease-out`. Wrap in `prefers-reduced-motion` check

**6.8.2 Video Player Section (P0 — The Main Event)**
- [ ] Player container: `rounded-xl overflow-hidden` with `aspect-video` (16:9). Max-height ~65vh on desktop so info + sidebar content visible below fold. Full-width on mobile
- [ ] Ambient glow behind player: `position: absolute; inset: -40px; z-index: -1`. Radial gradient using accent at very low opacity: `radial-gradient(ellipse, rgba(244,63,94,0.04), transparent 70%)`. Provides depth separation from page background. Delete if feels gimmicky during implementation
- [ ] Blurred background fill for non-16:9 content: behind the main video, render a `blur-2xl scale-110` version as ambient fill (same pattern as HeroSection's letterbox). Prevents black bars on vertical or oddly-cropped videos
- [ ] Control bar below player: `h-12 bg-surface-raised border-t border-surface-border px-4`. All icons are Lucide (zero unicode/emoji):
  - Lucide `Pause` / `Play` (toggle) — 20px
  - Lucide `SkipBack` — 16px (seek -10s)
  - Lucide `SkipForward` — 16px (seek +10s)
  - Progress scrubber (flex: 1): 4px height, `bg-surface-overlay`, filled portion `bg-accent`. Hover: grows to 6px, shows scrub thumb (12px circle, `bg-accent`, `opacity: 0 → 1, scale: 0.5 → 1`, 120ms)
  - Time display: `text-xs tabular-nums text-muted` ("3:24 / 9:47")
  - Lucide `Volume2` / `VolumeX` (mute toggle) — 18px
  - "HD" text button for quality selector — `text-xs font-semibold`
  - Lucide `PictureInPicture2` — 16px
  - Lucide `Maximize2` — 16px (fullscreen)
  - All control buttons: `w-9 h-9 rounded-lg bg-transparent text-secondary hover:bg-surface-overlay hover:text-primary active:scale-[0.9] focus-visible:ring-2 focus-visible:ring-accent/50 transition-all duration-150`
- [ ] Progress bar hover tooltip (desktop): small floating pill showing "1:23 / 4:56" at cursor position. `bg-surface-raised text-xs px-2 py-1 rounded`
- [ ] Keyboard shortcuts active immediately on page load (no need to click player first):
  - `Space` / `K` = play/pause (show animated Lucide `Pause`/`Play` icon, scale 0→1 then fade, 500ms, centered)
  - `J` = back 10s, `L` = forward 10s
  - `F` = fullscreen, `M` = mute
  - `ArrowLeft/Right` = seek 5s, `ArrowUp/Down` = volume 5%
  - `,` / `.` = frame step (when paused)
  - `Escape` = exit fullscreen, or if not fullscreen, navigate back
- [ ] Watch progress tracking: save to `watch_history` every 5s (increase to every 2s in the last 20% for accuracy near completion)

**6.8.3 Resume Overlay (P0 — Common State)**
- [ ] On page load, if video has prior watch progress >5% and <90%, show resume overlay on player:
  - Semi-transparent backdrop: `bg-black/70` covering the player area
  - Centered content stack: "Resume from 4:23" in `font-display text-lg text-primary` + large Lucide `Play` icon (48px circle, `bg-accent`) below + "Start over" as `text-sm text-secondary hover:text-primary` link underneath
  - Click play or click anywhere on backdrop: resume from saved position
  - Click "Start over": reset to 0:00 and play
  - Auto-dismiss after 5s if no interaction (resumes from saved position)
  - Animation: fade in 200ms on mount, fade out 200ms on dismiss
  - `aria-live="polite"` announcement: "Video paused at 4 minutes 23 seconds. Press Enter to resume or Tab to start over"

**6.8.4 Video Info Section (P0 — Context & Actions)**
- [ ] Info section: `py-5` below player container
- [ ] Title: `font-display text-[22px] font-semibold leading-tight mb-3 text-primary`
- [ ] Metadata row: `flex items-center gap-2 text-[13px] text-secondary mb-4`
  - Source badge: `text-[11px] font-semibold uppercase tracking-wide text-accent bg-accent-muted px-2 py-0.5 rounded` (shows domain: "YouTube", "RedGifs", etc.)
  - Uploader name (plain text)
  - Dot separator: `text-muted` ("·")
  - View count: compact format (1.2K, 45K, 1.2M) if available
  - Upload date: relative ("3 days ago") if available
- [ ] Action bar: `flex gap-[10px] pb-5 border-b border-surface-border`. All pill-shaped buttons:
  - **Like**: Lucide `Heart` (18px) + count. Default: `bg-surface-raised text-secondary`. Liked: `bg-accent-muted text-accent` + `heartPop` keyframe. Wired to `toggleFavorite`
  - **Queue**: Lucide `ListPlus` (18px) + "Queue". Shows toast on click
  - **Watch Later**: Lucide `Bookmark` (18px) + "Later"
  - **Share**: Lucide `Share2` (16px) + "Share". Copies URL (with `?t=currentTime` if mid-video) to clipboard
  - **Source**: Lucide `ExternalLink` (16px) + "Source". Opens original URL in new tab
  - Pill style: `px-4 py-[10px] rounded-full text-[13px] font-medium`. Min height 46px (44px touch target + padding)
  - States: default `bg-surface-raised text-secondary` → hover `bg-surface-overlay text-primary` → active `scale-[0.95]` (100ms) → focus `ring-2 ring-accent/50` → disabled `opacity-40 pointer-events-none`
  - On mobile: icon-only pills (no text labels), evenly spaced with `justify-around`
- [ ] Tag row: `flex flex-wrap gap-1.5 py-4`. Each tag: `text-xs px-3 py-1 rounded-xl bg-surface-raised text-secondary border border-surface-border hover:border-text-muted hover:text-primary active:scale-[0.97] active:bg-surface-overlay cursor-pointer transition-all duration-150`. Click navigates to `/library?tag=[tagname]`
- [ ] Description (collapsible): `text-sm text-secondary leading-relaxed`. Shows first 2 lines with `-webkit-line-clamp: 2`. "Show more" toggle: `text-primary font-medium text-[13px] hover:text-accent cursor-pointer`. If no description metadata, hide entirely (no empty state)

**6.8.5 Related Videos Sidebar (P0 — Discovery Engine / The Differentiator)**
- [ ] Sidebar container: `sticky top-20 max-h-[calc(100vh-100px)] overflow-y-auto pr-1`. Custom scrollbar: 4px width, `surface-border` thumb. Bottom mask: `mask-image: linear-gradient(to bottom, black 90%, transparent)` to indicate scroll continuation
- [ ] **Up Next card** (top of sidebar, always visible):
  - Container: `bg-surface-raised border border-surface-border rounded-xl p-4 mb-5 relative overflow-hidden`
  - Accent top border: `absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-accent to-transparent`
  - Label: `font-display text-[11px] font-semibold text-accent uppercase tracking-[1px]` + countdown: `font-sans text-[11px] text-muted font-normal normal-case tracking-normal` ("auto-playing in 10s")
  - Card layout: `flex gap-3`. Thumbnail: `w-40 min-w-40 aspect-video rounded-lg bg-surface-overlay` with duration badge. Title: `text-[13px] font-medium line-clamp-2`. Meta: `text-[11px] text-muted`
  - Hover: inner card gets `bg-surface-overlay rounded-lg` background
  - Source priority: 1) Next queue item, 2) Same uploader, 3) Highest tag overlap, 4) Trending in mode
  - Loading state: shimmer skeleton matching card dimensions
  - `aria-live="polite"` at 5s remaining: "Playing [title] in 5 seconds. Press Escape to cancel"
- [ ] **Related sections** (grouped with transparent reasoning — the differentiator):
  - Each section has a header: `text-xs font-medium text-secondary mb-3` + reason annotation in `text-[11px] text-muted`
  - Section headers explain WHY: "More from TaehaTypes" + "— same creator", "Because you watch" + tag pill, "Trending in" + category tag pill
  - Use `<h2>` elements for section headers (screen reader landmarks)
  - Compact related cards: `flex gap-[10px] p-[6px_8px] rounded-lg cursor-pointer mb-2`. Thumbnail: `w-[140px] min-w-[140px] aspect-video rounded-md bg-surface-overlay` with `box-shadow: inset 0 0 0 1px rgba(255,255,255,0.06)` (prevents bleeding into dark bg). Duration badge: `absolute bottom-1 right-1 text-[9px] font-semibold px-1 py-0.5 bg-black/80 rounded-sm`
  - Card info: title `text-xs font-medium line-clamp-2 text-primary mb-0.5`, meta `text-[11px] text-muted`
  - Card hover: `bg-surface-overlay` + thumbnail `brightness-110` filter. 150ms ease-out
  - Card active: `scale-[0.98]`. Card focus: `ring-2 ring-accent/50 rounded-lg`
  - 10-20 related videos loaded initially across all sections, infinite scroll for more
- [ ] Clicking a related video: `navigate('/watch/${newId}')`. Player crossfades (old fades 150ms, new thumbnail fades in). Info + sidebar update simultaneously (200ms slide-fade, not staggered). Scroll to top
- [ ] Mobile layout (<768px): no sidebar. Up Next card appears below action bar. Related sections stack below in full-width compact cards. Related section headers remain (reasoning still visible)
- [ ] **Related sidebar states:**
  - Loading: 3 skeleton cards per section matching related-card dimensions. Gradient sweep shimmer (per 6.3 upgrade, not pulse)
  - Empty (no tags, no related): single `StateMessage` component: Lucide `Compass` icon + "Explore more content" + CTA button linking to Feed page
  - Error (API failure): inline "Couldn't load related videos" + Lucide `RefreshCw` retry button
  - Up Next loading state: separate skeleton from related list (always shows independently)

**6.8.6 End-of-Video Behavior (P1 — Keep Them Watching)**
- [ ] On video end, show "Up Next" overlay on the player:
  - Semi-transparent backdrop: `bg-black/80`
  - Thumbnail of next video (large, ~40% of player width) + title + source badge
  - Circular countdown ring: SVG `stroke-dashoffset` animation over 10s, linear. Ring uses accent color, `48px` diameter
  - "Play Now" pill: `bg-accent text-white rounded-full px-6 py-2 font-medium hover:bg-accent-hover`
  - "Cancel" text link: `text-sm text-secondary hover:text-primary` — stops auto-advance, stays on current page
  - On countdown complete: Up Next card pulses `scale(1.02) → scale(1)` (200ms) then navigates
- [ ] Queue items override related for auto-advance. Queue empty: plays Up Next related video
- [ ] Nothing available (no queue, no related): show CTA "Add more sources" linking to Settings, never a dead screen
- [ ] Binge check: after 5 consecutive auto-advances, show "Still watching?" overlay. Pauses countdown. Lucide `Eye` icon + "Still there?" text + "Keep watching" button + "Take a break" link. `aria-live="assertive"`

**6.8.7 Theatre Mode Retirement (P1 — Remove Old Pattern)**
- [ ] Remove theatre mode from `HeroSection.jsx`. Hero becomes browse-only: show featured content, click navigates to `/watch/:id`. No inline playback
- [ ] Remove `TheatreControls.jsx` component (replaced by watch page player controls)
- [ ] Remove theatre-related state from `playerStore.js` (`isTheatreMode`, `theatreVideo`, etc.)
- [ ] Update `HeroSection` click handler: play button navigates to `/watch/:heroVideoId`
- [ ] Keep Ken Burns animation and visual design (billboard, not player)

**6.8.8 Queue Integration (P1 — Sidebar on Watch Page)**
- [ ] Show `FloatingQueue` on the watch page (same collapsed pill / expanded panel as other pages)
- [ ] When queue expanded on desktop: push related sidebar narrower or hide it, queue takes priority
- [ ] "Play next" and "Play last" options when adding to queue from watch page action bar
- [ ] Current video highlighted in queue with "Now Playing" accent indicator
- [ ] Drag-to-reorder within queue from watch page

**6.8.9 Deep Linking & Back Navigation (P2 — Polish)**
- [ ] URL structure: `/watch/:id` with optional `?t=123` for timestamp deep links
- [ ] Back button returns to previous page with scroll position preserved. `sessionStorage` scroll offset per route key
- [ ] Share button copies `/watch/:id?t=currentTime` to clipboard
- [ ] Unknown video ID: attempt metadata fetch via yt-dlp. Show page-level loading skeleton while fetching. If fetch fails: 404-style `StateMessage` with Lucide `FileQuestion` + "Video not found" + "Go home" CTA
- [ ] Browser history: each video navigation is a history entry for back/forward nav

**6.8.10 Backend: Related Videos Endpoint (P1 — New API)**
- [ ] Create `GET /api/videos/:id/related` endpoint in `server/index.js`:
  - Input: video ID
  - Query: pull video's tags, source, uploader from DB
  - Algorithm: weighted tier mix (Tier 1: same uploader weight 3.0, Tier 2: same tags + source weight 2.0, Tier 3: same category weight 1.5, Tier 4: trending fallback weight 1.0)
  - Response: `{ upNext: VideoObject, sections: [{ label, reason, videos: VideoObject[] }] }` — sections pre-grouped with labels/reasons so frontend doesn't need to sort
  - Exclude videos in watch history (unless library <50 videos)
  - Cache results 30 minutes per video ID
- [ ] Add `uploader` column to videos table if not present
- [ ] Add DB indexes on `videos.source` and `videos.uploader` for fast related queries

**6.8.11 Accessibility (P1 — Required)**
- [ ] Player controls: all reachable via Tab in logical order. Progress bar navigable with Arrow keys (5s per press). Volume with Up/Down when volume button focused
- [ ] Sidebar: `role="complementary" aria-label="Related videos"`. Related cards are `<a>` elements (not divs with onClick)
- [ ] All action pills: descriptive `aria-label` (e.g., "Like video, 45 thousand likes" not "♥ 45K")
- [ ] Up Next countdown: `aria-live="polite"` at 5s and on auto-advance
- [ ] `prefers-reduced-motion`: disable all translateY, scale animations. Keep opacity transitions. Progress bar: no grow-on-hover. Page enter: instant (no stagger). Video transition: instant cut (no crossfade)
- [ ] Contrast fix: `text-muted` (#8a8a90) on `surface` (#111113) is 4.2:1, below 4.5:1 for small text. Use `text-muted` only at `text-sm` (14px+) or bump to #9a9aa0 (~4.8:1) for smaller sizes

---

## Milestone 7: More Polish (Cross-Cutting Quality Pass)

> Goal: The stuff that separates "good app" from "app I tell people about." These are cross-cutting concerns that touch multiple pages and can't be solved per-component. Ship after M6 is mostly done.

### 7.1 Error & Empty State Design System

> Every failure point should feel designed, not accidental. One consistent pattern across the entire app.

- [ ] Create shared `<StateMessage>` component: icon (Lucide) + headline + body + optional CTA button. Used for all empty states, error states, and zero-data screens. Replaces the 5+ bespoke empty state implementations across LibraryPage tabs, feed exhausted, settings sections
- [ ] Create shared `<ErrorBoundary>` wrapper with fallback UI: "Something went wrong" + retry button + "Report issue" link. Currently uncaught errors white-screen the app
- [ ] Design 3 tiers of error severity with distinct visuals:
  - **Inline warning** (yellow accent, inline banner): non-blocking issues like "Source hasn't responded in 24h"
  - **Section error** (red accent, replaces section content): "Failed to load recommendations"
  - **Full-page error** (centered StateMessage): route-level failures, 404s, offline
- [ ] Network-down detection: listen for `navigator.onLine` changes, show a persistent top banner "You're offline — showing cached content" with accent color. Auto-dismiss when reconnected
- [ ] API timeout handling: if any fetch exceeds 10s, show inline skeleton + "Taking longer than usual..." message. Currently long loads show infinite spinners or nothing

### 7.2 Keyboard Shortcut System

> Power users live on the keyboard. Right now shortcuts exist but are scattered, undiscoverable, and inconsistent across pages.

- [ ] Create global `useKeyboardShortcuts` hook that registers all shortcuts in one place with conflict detection. Currently shortcuts are spread across FeedPage, VideoPlayer, LibraryPage, and AppShell with no central registry
- [ ] Add `?` shortcut to show keyboard shortcut overlay (like GitHub). Modal listing all active shortcuts grouped by context (Global, Feed, Player, Library). Dismisses on Escape or second `?`
- [ ] Standardize shortcut keys across pages:
  - `Space` = play/pause (everywhere, not just feed)
  - `F` = fullscreen / toggle theatre mode
  - `M` = mute/unmute
  - `L` = like/favorite (feed + library)
  - `Q` = toggle queue panel
  - `/` = focus search
  - `1-5` = star rating in library
- [ ] Add visual shortcut hints: on first visit to each page, show faded key hints next to actionable elements for 5s (e.g., "L" near the heart icon, "Q" near queue). Stores `fd-hints-seen-{page}` to not repeat

### 7.3 Toast & Notification System

> Currently toasts are per-page implementations with different animations, durations, and styles. Need one system.

- [ ] Create shared `<Toast>` system via Zustand store + portal renderer:
  - Types: `success` (green accent), `info` (blue), `warning` (yellow), `error` (red), `undo` (with action button)
  - Auto-dismiss: 3s for success/info, 5s for warning, persistent for error/undo
  - Stack up to 3 toasts, newest on top, older ones compress
  - Consistent animation: slide in from bottom-right (desktop) or top-center (mobile)
- [ ] Replace all existing toast implementations: `FeedToast.jsx`, inline toasts in SettingsPage, queue add confirmations
- [ ] Add undo pattern to destructive actions: toast with "Undo" button + countdown bar. On expiry, execute the action. On undo, restore state. Apply to: source delete, history clear, library remove

### 7.4 Accessibility Pass

> Not optional. Screen readers, keyboard nav, reduced motion, and contrast are table stakes.

- [ ] Audit all interactive elements for `aria-label`, `role`, and keyboard reachability. Priority targets: VideoCard actions (currently icon-only with no labels), FeedBottomNav tabs, mode toggle pill, TheatreControls buttons
- [ ] Add `aria-live="polite"` regions for dynamic content: toast notifications, feed video transitions, queue count changes, seed progress updates
- [ ] Respect `prefers-reduced-motion` comprehensively: currently `index.css` has a media query but it only disables some animations. Audit and disable: FeaturedSection scroll animation (show cards statically), CategoryRow stagger, Ken Burns hero, heart burst, queue pulse, all `transition` properties over 200ms
- [ ] Color contrast audit: verify all text/background combinations meet WCAG AA (4.5:1 for body, 3:1 for large text). Known risks: `text-white/60` on dark backgrounds, accent-on-surface combinations in light mode
- [ ] Skip navigation: `AppShell.jsx` has a skip-nav link — verify it targets the correct `main` element on every route and is visible on focus
- [ ] Focus trap for modals/sheets: `FeedFilterSheet`, `SourceControlSheet`, add-source modal, shortcut overlay. Tab should cycle within the open panel, not escape to background content

### 7.5 PWA & Offline Basics

> This will eventually live on a Raspberry Pi. PWA support gets it 80% of the way to feeling native without any native code.

- [ ] Add `manifest.json` with app name ("Puppy Gallery" in SFW, actual name configurable), icons, theme color, display: standalone. Register in `index.html`
- [ ] Add service worker with Workbox: cache app shell (HTML, CSS, JS bundles) for offline launch. Network-first for API calls, cache-first for static assets
- [ ] Add install prompt: detect `beforeinstallprompt` event, show a subtle "Install app" banner on 3rd visit. Dismiss permanently on decline
- [ ] Cache thumbnails for library items: when a video is favorited or added to Watch Later, cache its thumbnail in the service worker. Library page works offline with cached thumbnails + metadata from IndexedDB
- [ ] Offline feed: if network is down, show "Offline — browse your library" with a CTA to LibraryPage instead of a blank feed

### 7.6 Performance Budgets

> Fast is a feature. Set guardrails now before the codebase grows.

- [ ] Add Lighthouse CI to the build: fail if Performance score drops below 85, Accessibility below 90. Run on `npm run build` as a post-build check
- [ ] Set bundle size budget: main JS chunk <150KB gzipped, CSS <30KB. Add `bundlesize` or `size-limit` to CI. Current state unknown — measure first, then set targets 10% above baseline
- [ ] Lazy-load below-fold CategoryRows: only render rows visible in viewport + 1 buffer row. Use IntersectionObserver to mount/unmount. Currently all rows render on homepage mount
- [ ] Image optimization pipeline: ensure all thumbnails served as WebP with `srcset` for 1x/2x. Add `loading="lazy"` to all `<img>` tags below the fold. Verify `useHoverPreview` doesn't load full-res video for hover previews
- [ ] Audit re-renders: add React DevTools Profiler pass. Known suspects: `FeedPage` (10+ state variables causing cascading renders), `LibraryPage` (full re-render on tab switch), `HeroSection` (scroll handler on every frame)
- [ ] Memory leak cleanup: fix the 54 orphaned `<video>` elements from hover previews (Discovered Tasks item). Add `useEffect` cleanup in `useHoverPreview` that removes the singleton element on unmount

### 7.7 Transition & Navigation Polish

> The space between pages matters as much as the pages themselves.

- [ ] Consistent page enter animation: every route fades in with a 150ms opacity transition. Use CSS View Transitions API where supported (cross-ref 6.3 task), fallback to `useLocation` key-based CSS transition
- [ ] Shared element transitions: when clicking a VideoCard thumbnail, the thumbnail should animate (scale + translate) from its grid position to the player position. CSS `view-transition-name` on matching elements
- [ ] Navigation loading states: if a route takes >200ms to load (lazy chunk), show a thin progress bar at the top of the viewport (like YouTube's red loading bar). Currently route changes are instant-or-nothing
- [ ] Back navigation memory: when returning to LibraryPage or HomePage from a detail view, restore scroll position. Currently resets to top. Use `useScrollRestoration` or manual `sessionStorage` of scroll offset per route
- [ ] Queue panel transition: `FloatingQueue` expand/collapse should use a spring animation (not linear). Collapsed pill slides up and morphs into panel, rather than the current instant appear/disappear

---

## Discovered Tasks

_Claude Code adds tasks here as they come up during implementation. Move to the appropriate section when triaging._

- [x] Reduce CDN URL cache TTL from 4 hours to 2 hours (PornHub URLs expire in ~2hr)
- [x] Wire source adapter system (`server/sources/`) into `server/index.js` — completed in 3.0 Integration
- [x] Clean up 3 stale `vite.config.js.timestamp-*` files in project root
- [x] **CRITICAL:** Fix missing `crypto` import in `server/index.js` — playlist creation crashed at runtime. Fixed: imported `randomBytes` from `crypto` (Cowork morning sprint 2026-03-22)
- [x] Wire tag preferences into `refillCategory()` and `_refillFeedCacheImpl()` — both now query liked tags and append up to 2 random liked tags to search queries for personalized discovery. Discovered during personalization audit (morning sprint 2026-03-22)
- [ ] Hover preview video element cleanup — found 54 `<video>` elements in DOM, likely from hover previews not being properly destroyed. Potential memory leak. Discovered during 5a.2 playback testing
- [x] **HIGH:** Add timeout to yt-dlp `streamSearch()` spawn — 60s kill timer prevents leaked processes (Cowork morning sprint 2026-03-22)
- [x] **HIGH:** Cap feed buffer at 200 items with safe eviction in `feedStore.js` — prevents OOM on long sessions (Cowork morning sprint 2026-03-22)
- [ ] **HIGH:** Close Puppeteer browser on scrape failure in `server/sources/scraper.js` (~line 195) — failed scrapes leave browser instances alive
- [ ] Add SIGTERM handler to clear background `setInterval` callbacks in `server/index.js` (~lines 1392, 1423, 1460) and close DB
- [ ] Add per-chunk timeout to proxy-stream pipe in `server/index.js` (~line 240) — stalled upstream blocks response forever
- [ ] Add AbortController to `_warmStreamUrls()` in feedStore, abort on `resetFeed()` — fire-and-forget fetches update stale buffer
- [ ] Log malformed JSON parse failures in `server/index.js` tag processing instead of silently skipping
- [ ] Wire tag preferences into `refillCategory()` — currently uses hardcoded generic queries, ignoring liked/disliked tags entirely
- [ ] Remaining 16 `react-hooks/exhaustive-deps` ESLint warnings — need per-hook manual review to avoid infinite loops
- [!] **HIGH:** Content spillage between NSFW and SFW modes — combined `cookies.txt` sends PornHub cookies on YouTube requests and vice versa. Tag preferences from both modes are mixed into one pool (e.g., "amateur", "big tits" tags influence Social mode discovery). Needs: (1) per-mode cookie files (3.4.1), (2) mode-aware tag preferences so NSFW tags don't leak into Social queries
  > BLOCKED: Requires 3.4.1 (per-mode cookie files) + new `mode` column on `tag_preferences` table
- [x] Remove debug `console.log('Queue: advancing to')` from `VideoPlayer.jsx:136` and `useKeyboard.js:41` (Cowork morning sprint 2026-03-22)

---

## Completed

- [x] (2026-03-19) Initialize Vite + React project with folder structure
- [x] (2026-03-19) Configure Tailwind CSS with dark theme and custom design tokens
- [x] (2026-03-19) Set up Zustand stores (mode, queue, library)
- [x] (2026-03-19) Create Express server with health check and API routes
- [x] (2026-03-19) Initialize SQLite database with videos, preferences, history tables
- [x] (2026-03-19) Configure Vite proxy for /api requests
- [x] (2026-03-19) Add npm scripts (dev, build, preview)
- [x] (2026-03-19) SFW/NSFW mode store with localStorage persistence
- [x] (2026-03-19) Escape key panic handler (always to SFW)
- [x] (2026-03-19) Visual mode toggle button
- [x] (2026-03-19) Tab title and favicon swap on mode change
- [x] (2026-03-19) SFW data helper with deterministic fake metadata per video ID
- [x] (2026-03-19) Conditional render for all thumbnails, titles, metadata
- [x] (2026-03-19) Desktop thumbnail grid (responsive 2/3/4/5 columns)
- [x] (2026-03-19) Thumbnail card with duration badge
- [x] (2026-03-19) Lazy loading images
- [x] (2026-03-19) Video player with standard controls
- [x] (2026-03-19) Keyboard shortcuts (Space, arrows, F, M, N)
- [x] (2026-03-19) Queue sidebar with add/remove/reorder
- [x] (2026-03-19) Queue persistence in localStorage
- [x] (2026-03-19) Header with search and mode toggle
- [x] (2026-03-19) Add video modal with URL paste
- [x] (2026-03-19) Backend yt-dlp metadata extraction endpoint
- [x] (2026-03-19) Backend stream URL endpoint
- [x] (2026-03-19) yt-dlp validated with Arc browser cookies
- [x] (2026-03-19) Context menu on thumbnails
- [x] (2026-03-20) Homepage mockup built (HTML prototype, reference only)
- [x] (2026-03-20) Homepage design spec with state machine, component tree, store design
- [x] (2026-03-20) Design tokens finalized in tailwind.config.js
- [x] (2026-03-20) All Priority 1 scroll animation tasks (except real-browser testing)
- [x] (2026-03-20) All Priority 2 React components built
- [x] (2026-03-20) Test in real browser and fine-tune phase boundaries/timing — rewrote entire scroll animation: replaced state machine with CSS sticky + 5-phase scroll-position interpolation (modeled on Apple TV). Files: `useFeaturedScroll.js` (rewritten), `FeaturedSection.jsx` (350vh zone, overlay ref, Phase 2 video playback)
- [x] (2026-03-20) Create `playerStore.js` for shared activeVideo state across hero, carousel, queue — already existed from prior work
- [x] (2026-03-20) Theatre mode end-to-end: hero video plays, TheatreControls wired to playerStore, FeaturedSection+CategoryRows hide, exit restores
- [x] (2026-03-20) Hero ↔ Carousel interaction: clicking carousel card updates hero — already worked via setHeroItem
- [x] (2026-03-20) Verify all keyboard shortcuts work on HomePage — added theatre mode keyboard handler (Space, arrows, F, M) to HeroSection
- [x] (2026-03-20) Add `homepage_cache` table — already existed in database.js
- [x] (2026-03-20) Add `categories` table — already existed with seed data in database.js
- [x] (2026-03-20) `GET /api/homepage?mode=social|nsfw` — already implemented in server/index.js
- [x] (2026-03-20) Async yt-dlp refill when category below 8 — already implemented in refillCategory()
- [x] (2026-03-20) `POST /api/homepage/viewed?id=` — already implemented with refill trigger
- [x] (2026-03-20) Homepage → real data bridge: fetchHomepage() in homeStore calls API, maps response, falls back to placeholders when cache empty
- [x] (2026-03-20) Hover preview: useHoverPreview hook with 300ms debounce, singleton abort, muted video overlay on CategoryRow and VideoCard
- [x] (2026-03-20) Stale URL auto-retry: handleStreamError in playerStore, onError handler on theatre video element
- [x] (2026-03-20) yt-dlp error handling: backend parses unavailable/geo-blocked/rate-limited errors, frontend shows via streamError
- [x] (2026-03-20) Loading skeletons: SkeletonHero, SkeletonFeatured, SkeletonCategoryRow, SkeletonVideoGrid in Skeletons.jsx, wired into HomePage
- [x] (2026-03-20) Empty states: CategoryRows shows message when no categories, VideoGrid already had EmptyState, queue already had empty state
- [x] (2026-03-20) Rebrand: localStorage keys pv-* → fd-*, migration in migrations.js (runs before store init), all Zustand store names updated
- [x] (2026-03-20) Rebrand: deleted puppyData.js, replaced all imports with socialData.js across 6+ components
- [x] (2026-03-20) Rebrand: UI text updated — Header, ModeToggle, VideoGrid, QueueSidebar, dist/index.html, modeStore, useAppStore
- [x] (2026-03-20) Rebrand: package.json name → "feeddeck", launch.json configs renamed
- [x] (2026-03-20) Responsive breakpoint audit: verified homepage renders at sm/md/lg/xl breakpoints
- [x] (2026-03-20) Next-up preview overlay: shows upcoming queue item in corner during last 10s of theatre playback
- [x] (2026-03-20) Playback speed selector: SpeedSelector component in TheatreControls, cycles 0.5x–2x
- [x] (2026-03-20) Hero search bar: narrowed to 280px, helper text moved below input
- [x] (2026-03-21) Featured carousel overlay: replaced React inline styles with Tailwind classes so scroll hook's imperative DOM updates survive re-renders; added setOverlayOpacity(1) to applyCarousel()
- [x] (2026-03-21) Featured carousel video preview: stream URL fetch + auto-play on canplay, onPlaying/onPause fade video in/out
- [x] (2026-03-21) Removed React.StrictMode to fix competing scroll handlers from double-mount
- [x] (2026-03-21) M2 Feed Core (2.1): route, page, store, video component, backend tables/endpoints, content pipeline
- [x] (2026-03-21) M2 Gesture System (2.2): touch swipes, double-tap hearts, long-press source sheet, desktop keyboard
- [x] (2026-03-21) M2 Feed UI (2.3): FeedVideo, FeedToast, HeartBurst, SourceControlSheet, FeedBottomNav, QueueSwipeAnimation
- [x] (2026-03-21) M2 Feed Store (2.4): feedStore.js with buffer, index, watched tracking, letterbox preference
- [x] (2026-03-21) M2 Feed Backend (2.5): feed_cache/sources tables, API endpoints, scheduled refill, stream URL caching
- [x] (2026-03-21) M2 Mobile Setup (2.6): Vite + Express bound to 0.0.0.0, network URL printed on startup
- [x] (2026-03-21) M2 Feed Polish (2.7): pull-to-refresh, loading spinner, reduced-motion support, orientation crossfade
- [x] (2026-03-21) Fixed mobile video playback: switched from HLS to direct MP4 format IDs (480p/240p/720p), routed all CDN URLs through /api/proxy-stream to avoid ORB blocking
- [x] (2026-03-21) Unmute persistence working: shared singleton video element preserves gesture activation across swipes
- [x] (2026-03-21) Security: fixed command injection (execFile instead of shell interpolation), SSRF protection (CDN domain whitelist), crash-on-rejection handler
- [x] (2026-03-21) Performance: async yt-dlp in /api/health and /api/metadata (was blocking event loop)
- [x] (2026-03-21) Code cleanup: removed dead stores (useAppStore, useContentStore), dead components (App.jsx, Queue.jsx), unused dep (react-swipeable), fixed stale closure in FeedVideo, debug overlay behind DEV flag, HLS proxy URL rewriting
- [x] (2026-03-21) 2.8 Tier 1: Pre-resolve stream URLs at ingest time — `_preResolveStreamUrls()` in server/index.js resolves URLs during background refill
- [x] (2026-03-21) 2.8 Tier 1: Pre-warm feed on app load — `feedStore.prefetch()` via `requestIdleCallback` on homepage
- [x] (2026-03-21) 2.8 Tier 1: Eager URL warm-up — `_warmStreamUrls()` resolves stream URLs for new buffer videos and updates store
- [x] (2026-03-21) 2.8 Tier 2: Adaptive preload window — `_getPreloadWindow()` uses Network Information API (4g=4, 3g=2, 2g=1, default=2-3)
- [x] (2026-03-21) 2.8 Tier 2: Preconnect hints — N/A, all loads proxied through same-origin
- [x] (2026-03-21) 2.8 Tier 2: Video element pooling — deferred, conflicts with singleton unmute pattern for iOS
- [x] (2026-03-21) 2.9: Long-press timing fix (500ms → 800ms)
- [x] (2026-03-21) 2.9: Refresh feed button (top-right, visible at first video)
- [x] (2026-03-21) 2.9: Fullscreen immersive mode (toggle button, overlay auto-hides after 3s, tap to reveal)
- [x] (2026-03-21) 2.9: Auto-hide nav bar (scroll-direction detection, CSS transform transition)
- [x] (2026-03-21) 2.9: Settings access rethink — three-dot source control button on active video, tap-friendly alternative to long-press
- [x] (2026-03-22) 3.1 Queue Sync: SQLite-backed queue with CRUD API, queueStore migrated from localStorage to server, useQueueSync polling hook (3s), OfflineBanner
- [x] (2026-03-22) 3.2 Tag Preferences: tag_preferences table, settings UI with liked/disliked chips, /api/tags/* endpoints
- [x] (2026-03-22) 3.3 Basic Recommendations: /api/discover endpoint with rule-based scoring, /api/tags/popular
- [x] (2026-03-22) 3.4 Cookie Auth: cookies.txt import via settings page, yt-dlp auto-detects cookies file, status indicator
- [x] (2026-03-22) 3.5 Organization: favorites toggle, watch later, star ratings (1-5), custom playlists (full CRUD), library filter tabs
- [x] (2026-03-22) 3.6 Search: HeroCarousel wired to /api/search/multi (multi-site parallel search)
- [x] (2026-03-22) 3.7 PiP: native browser PiP button + P keyboard shortcut
- [x] (2026-03-22) 3.8 Quality Selector: /api/stream-formats endpoint, quality dropdown in VideoPlayer, localStorage preference
- [x] (2026-03-22) 3.9 Hero Image Fitting: object-contain with blurred background fill + radial gradient vignette
- [x] (2026-03-22) 4.1 Deployment, 4.3 Theme, 4.8 Source Management (committed from previous session)
- [x] (2026-03-22) 5.9 Library Page Upgrade: font-display headers, tab bar (All/Favorites/History/Watch Later/Top Rated) with count badges, Continue Watching horizontal row with progress bars, per-tab empty states with CTAs, watchProgress tracking in libraryStore
- [x] (2026-03-22) 5a.1 Playback Audit & Fixes: HLS.js fatal error recovery (reject instead of resolve), HeroSection onEnded queue autoadvance, proxy-stream 15s timeout, stream URL expires_at check, proactive TTL monitor (re-resolves URLs expiring within 15min)
- [x] (2026-03-22) 2.8 Tier 3: Stream URL TTL monitoring with proactive re-resolution, /api/stream-url now checks expires_at
- [x] (2026-03-22) Homepage playback fix: all CDN stream URLs now routed through /api/proxy-stream (HeroSection theatre mode + FeaturedSection previews were using raw CDN URLs that failed due to CORS/Referer requirements)
